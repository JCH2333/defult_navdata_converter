...
- 2026-08-14 的 `r54` 只读来源缺口审计以与 `r53` 覆盖 BGL 相同的 `r51`
  读取器 SQLite 对照受控参考 SQLite：1,109 个参考缺失航点中，702 个不在当期
  `DESIGNATED_POINT.csv` 或 `RTE_SEG.csv` 端点中，372 个仅以不同区域键出现在
  指定点表中，15 个指定点区域仍未决，20 个仅以不同区域键出现在航路端点中。
  这些分类只能用于继续回到 424 取证，不能作为重区域化或新增的依据。1,215 个
  参考缺失航路中，608 个可按 `RTE_SEG.csv` 的航路名和序号找到同源段，123 个仅
  能匹配航路名但序号不同，484 个不在当期 `RTE_SEG.csv` 中；当前来源没有可证明
  的默认 BGL 片段编号规则，不能依据参考身份反推 `airway_fragment_no`。
- 同次审计确认 246 个参考缺失的全局航点身份可在 424 终端坐标页中唯一对应，但
  这些资料共包含 11,981 个终端全局身份，其中 349 个在同一坐标被多个机场使用，
  其余绝大多数只服务于单一机场。现有资料尚不能仅凭终端坐标页区分应写入
  `00_enroute.bgl` 的少数点与只应保留在机场 BGL 的点；不得把参考中的 246 个
  身份作为投影名单，也不得把全部终端航点写入 enroute 覆盖层。
