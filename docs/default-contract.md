...
- 2026-08-14 对 `FLIGHT_AIRLINE_POINT.csv` 和完整终端资料的补充审计表明：
  前者只有名称、标识、频道/频率和 UUID，没有设施类型、坐标或区域；后者中少数
  同标识只作为裸程序固定点出现，未同时给出类型、区域和坐标。两者均不能作为
  缺失 VOR/NDB 的独立投影依据。
- AD 2.19 机场无线电导航设施表可提供直接打印的 VOR/DME 标识、频率、坐标和
  DME 天线高程，并作为带页码与 SHA-256 的审计证据保留。它没有可验证的设施
  ...
  CZW 等行的 `013°MAG/2000m` 是天线相对位置而非磁差，不能误用。
