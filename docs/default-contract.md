...
- 743 个 IAP 程序分组中，665 个具有唯一且非空的主进近数据库编码腿；其中 606 个
  分组的仪表进近图角色已由唯一图页或唯一 MAP/MAPT 终点安全消歧，1 个分组有唯一
  图页但没有可用角色标记，56 个分组仍有多图歧义，2 个分组没有匹配图页，78 个
  分组没有唯一主进近编码段。报告将角色证据、完整数据库腿和未决分组分开统计，
...
IAP 覆盖报告由 `iap_coverage.version=1` 标记。所有角色投影必须满足：数据库编码页
提供有序腿，仪表进近图页提供明确角色，且同一机场、跑道、程序标签的图页唯一；多图
时只有主进近最后定位点在恰好一张图上明确为 `MAP/MAPT` 才允许消歧。其余分组仍可
保留已解析的来源腿，但必须在 `unresolved_groups` 中列出，不能猜测图页或从参考
成品回填程序语义。
