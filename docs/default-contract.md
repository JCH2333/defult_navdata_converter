...
`airport_subset_probe.py` 必须在每次诊断目录写入 `probe-report.json`，保存完整输入选择、源 XML/生成 XML/探针脚本的大小与 SHA-256、Package Tool 编译器路径/命令/尝试/进程轨迹/Builder log、完整包树的大小与 SHA-256、编译产物根目录、读取器状态，以及每个 BGL 的文件大小、头部版本、QMID 和节表类型/计数/尺寸；不读取任何参考 BGL 记录。2026-08-19 对 `ZUAL` 的受控 Package Tool 构建表明：只保留跑道时节为 `0x3/0x13/0x32/0x35`、计数均为 1；保留机场内终端点、程序和等待航线时出现 `0x22`/`0x34`，计数为 `1/1/10/1/1/1`；再保留根节点终端点时仅将 `0x22` 提升到 153，仍不产生参考机场 BGL 的 `0x17/0x33`。因此根节点终端点重复写入不是参考索引节差异的充分解释，后续必须继续通过隔离 SDK 探针验证其他可控输入。回归：`test_probe_layout_summary_reads_only_bgl_headers`、`test_probe_report_is_persisted_as_utf8_json`、`test_probe_file_descriptions_use_content_hashes_and_stable_tree_paths`。

2026-08-19 的 r140/r141 扩展探针以属性型 `--append-airport-child TAG;NAME=VALUE` 与 `--append-root-child TAG;NAME=VALUE` 受控附加 SDK 子对象，且两者永远不进入 `NavModel` 或正式候选。对同一份 `ZUAL` 输入，`Com`、`Tower`、`Start` 与完整的 `RunwayAlias` 均保持 `0x3/0x13/0x22/0x32/0x34/0x35`；一个完整属性的机场内或根节点 `Ndb` 均会稳定增加 `0x17` 与 `0x33`，各增加一条，故对象位置不能解释参考计数差异。叠加 `onlyAddIfReplace=TRUE` 后节类型可变为参考同类集合 `0x3/0x13/0x17/0x22/0x32/0x33/0x34`，但该标记不能进入通用投影：同日对新增机场 `ZBCF` 的隔离包经 Navdatareader 读取，机场与跑道记录均为 0，只剩航点与 ILS。因此它只可作为布局诊断，不构成运行时兼容规则。NDB 结果也只证明该对象是两个节的充分触发条件，不证明参考机场 BGL 的数千条索引记录来自 424 机场关联 NDB：后续 r388 以完整 `NDB.csv` 统计为 77 条，仍不足以解释参考各分区的 2,003 至 3,614 条节计数。因此不得据此把 NDB 接入机场投影，除非另有独立的 424 来源规则同时解释记录数量、作用域与加载契约。回归：`test_airport_child_specs_are_attribute_only_and_append_in_order`、`test_root_children_reuse_diagnostic_specs_without_reparenting`。
2026-08-21 的 r388 将上述结论固化为可复用 `sdk-section-closure-audit`：它只消费 Section provenance、SDK 表达矩阵、来源完整性和机场来源库存，不读取参考 payload。r311 的 NDB 触发效果可复现，但当前来源库存按完整 `NDB.csv` 统计为 77 条，仍不能解释参考机场 Section 的作用域和数千级基数；r383 也报告没有来源完整且未测试的 SDK 探针候选。因此 `0x17/0x33` 与 `0x35` 均不得进入正式 adapter，报告 `projection_authorized=false`。回归：`tests/test_sdk_section_closure_audit.py`。
2026-08-21 的 r389 新增 `airport-document-source-audit`，以固定 `b10331`、seed `2608`、temperature `0` 的 OCR 运行时读取 `机场_1.3.1机场和直升机场索引.pdf`、`机场_1.3.2机场一览表.pdf` 和 `机场_1.3.3机场飞行程序和运行最低标准联系信息表.pdf`。参考额外机场 `ZBSH`、`ZGFS`、`ZL02`、`ZL03`、`ZSLT`、`ZW01`、`ZW02` 均未在三份 OCR 文档中命中；即使命中，机场 PDF 也不能单独提供 `AD_HP`、跑道、终端和目标加载契约，因此报告固定为 `projection_authorized=false`，不得把索引资料硬编码进 `NavModel`。证据：`diagnostics/r389-airport-document-source-audit.json`；回归：`tests/test_airport_document_source_audit.py`。

2026-08-19 的 r195 将 `airport-source-inventory` 升级为 v2：它除已有来源对象外，还必须列出“直接 424 字段 -> SDK 子对象”的候选和已否决方向。`RWY_DIRECTION.csv.VAL_THR_DISPLACE` 有 `33` 条正值，可直接对应 SDK `Runway/OffsetThreshold`，因此是唯一 `eligible_for_sdk_probe` 的跑道子对象；`RWY.CODE_COMPOSITION` 已由 r194 否决为仅影响既有跑道编码，机场关联 VOR/NDB 与 `AIRSPACE_RADIO.csv` 继续分别因作用域不足而拒绝。探针以 `--keep-runway-number` 只保留既有物理跑道，并以 `--append-runway-child` 只向指定跑道插入子对象；`OffsetThreshold`、`BlastPad`、`Overrun` 必须插在 `Ils`/`IlsReference` 前以遵守 `ctRunway` XSD 顺序。r195 对 `ZPPP/04R`（`RWY_DIRECTION.csv:65`，`VAL_THR_DISPLACE=300m`）使用 `OffsetThreshold end=PRIMARY,length=984F,surface=CONCRETE`：输入 XML 从 `1377` 增至 `1443` 字节，BGL 从 `846` 增至 `878` 字节，两个 BGL 都保持 `0x3/0x13/0x32/0x35` 各一条，读取器均登记 `bgl_file_rows=1`、`ils=2`。该字段可被 SDK 合法编码，但不能解释参考缺失的 `0x17/0x33`，也未提高参考 `29` 文件的 SHA-256 一致数；不得接入正式 adapter。回归：`test_inventory_lists_direct_runway_offset_threshold_probe_candidates`、`test_runway_offset_threshold_is_inserted_before_existing_ils`、`test_keep_runways_rejects_unknown_and_preserves_source_order`。
...
2026-08-18 证实，组合标题“最后进近、复飞”的分界不只会表现为紧接
`CA`/`CF`/`DF`；也可以由当前跑道的 `TF RWxx` 终止进近后，以
`TF`/`RF` 开始复飞段。该规则变动已将缓存版本从 `40` 递增至 `41`；
回归测试为 `test_database_final_approach_missed_splits_after_runway_termination`。

IAP 覆盖报告由 `iap_coverage.version=15` 标记。所有角色投影必须满足：数据库编码页
...
当多张候选图都是不含 `ILS` 的纯 RNP 图、所有候选的 `(AR)` 属性一致，且每一张对
数据库主进近腿给出完全相同、非空的直接 `IAF`、`IF`、`FAF`、`MAP` 或 `MAPT` 角色时，
可投影这些共同角色，而不选择或假定任一图页对应具体变体。非 AR 候选的标准化标题必须
彼此不同；AR 候选允许相同标题。混入 ILS、混合 AR/非 AR、非 AR 重复标题、角色集合
为空或不一致时一律继续拒绝。选择记录为
`source_consensus_direct_role_selections`；它只读取 424 直接图页文本，不使用 OCR、
参考成品或 Fenix。自动化测试：
`test_iap_coverage_projects_identical_direct_roles_without_selecting_a_variant`、
`test_iap_coverage_projects_identical_non_ar_rnp_direct_roles_without_selecting_a_variant`、
...
当其余直接角色、固定点和标题规则均不能选择，但来源主进近的第一条腿明确为 `IF`，
且恰好一张候选图把该固定点直接标为 `IF` 时，可选择该图并记录为
`source_unique_first_if_selections`。首腿不是 `IF`、首腿固定点为空或多张图命中时一律
拒绝。该规则仅使用来源腿顺序和图页直接角色，不读取 OCR、参考成品或 Fenix。自动化
测试：`test_iap_coverage_selects_unique_first_source_if_chart` 与
...
无后缀主进近也可以被同页的 `Rxx-Y`/`Rxx-Z` 复飞变体继承。只有后缀标签恰好为无后缀身份加单个 `W/X/Y/Z`、该后缀组没有主进近、无后缀组有且仅有一个非空主进近、且全部后缀段与该主段位于同一数据库页时，才写入 `inherited_base_primary_assignments` 并投影带后缀的进近。跨页、复合后缀或后缀组已有主段必须继续拒绝。
...
- 当前候选已经可以生成两个具有 BGL、`bglIndex.bout`、`layout.json`、
  ...
  收敛。
- 2026-08-19 的 r136 使用经当前 `navigraph-nav-base`/`navigraph-nav-jepp`
  树指纹验证的官方导航台索引，只将其用于物理冲突识别、官方 NDB 保留和区域键恢复；
  所有投影字段仍来自 424。该构建选择 254 个来源可审计的 VOR/NDB，令
  `00_enroute.bgl` 的节类型从 `0x20/0x22/0x34` 收敛为
  `0x13/0x17/0x20/0x22/0x32/0x33/0x34`，与参考布局类型一致。剩余节计数或
  文件大小差异只能作为继续追溯 424 来源与 SDK 编译行为的信号，不得按参考 BGL
  反向补写实体或字段。
- 2026-08-19 的 `source-gap-audit-v5` 在原有参考独有航点/航路来源分类之外，
  新增航路字段差异的来源画像：只聚合完整脱敏差分中同逻辑键航段的变化字段名，以及
  对应 424 航段是否已连接 `SEGMENT`/`EN_ROUTE_RTE`、是否带 MTCA、PBN、方向、
  备注和完整端点键。报告不输出参考值、航路身份或原始字段值，不能作为反向补值通道；
  它用于确定某一候选字段映射是否已有可复用的来源证据。
- 2026-08-19 的 r136 `source-gap-audit-v4` 以完整、脱敏的 r136 单
  `00_enroute.bgl` 语义差分作为只读输入：4,446 条 424 航路中已有 4,434 条写入
  候选 XML，12 条因端点区域键为空被拒绝。1,177 条参考独有航路身份中，557 条已能
  对应相同的 424 航路名和序号且候选端点对已投影，属于表示或字段差异而不是来源缺失；
  其余分类为 13 条端点区域缺失、123 条同名不同序号和 484 条当前
  `RTE_SEG.csv` 不存在。后续只能从这些分类追溯 424 规则，不能按参考身份补线。
- 2026-08-19 的 r137/r138 验证了 GeneralDoc 缓存的适配器隔离。完整 ENR 3.2
  A/B/G/H/J/R/V/W/X 缓存可解析 455 页、4,069 条发布最低高度，其中 3,827 条唯一
  回链到直接 `RTE_SEG.csv`，242 条无直接航段，歧义和冲突均为零。r137 显式传入
  `--general-doc-airway-cache-directories` 后，航路最低高度字段差异由 r136 的 30 条
  增至 3,044 条，严格相等行由 2,688 降至 1,625；因此该发布高度不能在没有默认 BGL
  加载契约证据时作为默认适配器的投影输入。r138 使用同一经校验的 GeneralDoc 缓存但
  不传入 ENR 3.2 目录，保留 ENR 4.4 的 489 个安全关键点投影，单
  `00_enroute.bgl` 航点为 `3150/3266`、严格相等行提升至 2,699，最低高度字段差异
  维持 30 条。ENR 3.2 证据必须继续保留在可复用 `NavModel` 中供其他目标适配器按其
  自身契约选择；默认 BGL 候选不得隐式启用它，也不得据此新增航路、端点或区域键。
- 2026-08-19 的 r155 中间模型快照构建复核了航路点身份假设：4,446 条
  `RTE_SEG.csv` 航路段的已解析端点中，不存在“同区域、同 SDK 规范化标识、不同坐标”的
  物理身份冲突；r138 与 r155 的 `china-navdata.xml` 均为 3,150 个航点、8,868 条
  航路链接且集合完全一致。不得为默认 BGL 或其他适配器凭空附加 `001` 等标识后缀；
  将不同来源的同名点改名必须先取得目标格式的明确唯一身份契约。中间模型快照仍可作为
  后续适配器的固定输入，避免再次解析 424 或读取 Fenix 数据。
