...
对于 2608 `DESIGNATED_POINT.csv` 中 `CODE_FIR` 为空的指定点，源加载器只在
`SERVICED_AIRPORT` 严格匹配 `Z[A-Z]{3}` 的四位中国 ICAO 代码时使用其前两位
作为区域键。该规则只生成区域键，不复制官方航点记录；已有 `CODE_FIR` 和明确的
边界点覆盖规则优先。当前 2608 审计中，12 条空 FIR 且带服务机场的记录有 10 条
可被已验证官方航点索引唯一匹配，且区域均与服务机场前缀一致；`P216 -> ZUHY`
...
424 的 VOR/NDB `CODE_FIR` 是单一情报区时，默认通用数据适配器以其映射的区域键为准；服务机场 ICAO 前缀只在 `CODE_FIR` 为空时兜底。若 `CODE_FIR` 明确列出多个情报区，则不擅自选择第一个区域，而使用完整服务机场提供的边界侧；仍无法唯一确定时拒绝该记录。该规则仅使用当期 424 字段。
