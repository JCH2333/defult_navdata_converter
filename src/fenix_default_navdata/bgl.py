...
def _waypoint_matches_terminal_point(point, waypoints: list) -> bool:
    region = (point.country or point.airport[:2]).upper()[:2]
    return any(
        wp.ident.upper() == point.ident.upper()
        and (not wp.country or wp.country.upper()[:2] == region)
        and _great_circle_distance_nm(
            point.latitude,
            point.longitude,
            wp.latitude,
...
def _resolve_source_fix(
    ...
    if waypoint is not None and (
        not region
        or waypoint.country.upper()[:2] == region.upper()[:2]
    ):
...
def _append_enroute(
    ...
    baseline_navaids = tuple(
        official_overlay.canonicalize_navaid(navaid)
        if official_overlay is not None
        else navaid
        for navaid in candidate_navaids
    )
    official_canonicalized_navaids = sum(
        original != canonical
        for original, canonical in zip(candidate_navaids, baseline_navaids)
    )
    navaid_coordinates = {
    ...
    official_overlapping_airway_legs_retained = (
        sum(
            official_overlay.has_official_airway_edge(leg.airway, leg)
    ...
    return (
        ...
        official_canonicalized_navaids,
        official_overlapping_airway_legs_retained,
    )
    ...
    *,
    ...
    for airport in projected_airports:
        ...
        terminal_points = sorted(
            (
                ...
                and not _waypoint_matches_terminal_point(
                    point,
                    model.waypoints,
                )
            ),
        ...
        if duplicate_terminal_waypoints:
            ...
            for point_key, point in sorted(
                ...
                if terminal_identities[point_key] not in root_terminal_idents:
                    continue
                if _navaid_matches_terminal_point(point, model.navaids) or _waypoint_matches_terminal_point(point, model.waypoints):
                    continue
                root_identity = (
    ...
    if (
        ...
        root_terminal_waypoint_count = sum(
            not _navaid_matches_terminal_point(point, model.navaids) and not _waypoint_matches_terminal_point(point, model.waypoints)
    ...
    skipped_airway_leg_details: tuple[dict[str, object], ...] = ()
    official_canonicalized_navaids = 0
    official_overlapping_airway_legs_retained = 0
    if scope in {"all", "enroute"}:
        (
            enroute_waypoints,
    ...
    return XmlProjection(
        ...
        official_canonicalized_navaids=official_canonicalized_navaids,
        official_overlapping_airway_legs_retained=(
            official_overlapping_airway_legs_retained
        ),
