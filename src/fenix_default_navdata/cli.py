...
from .pipeline_master_audit import (
...
from .adapter_increment_authorization_audit import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    adapter_increment = sub.add_parser(
        "adapter-increment-authorization-audit",
        help="???????? 424 ???????????????????",
    )
    adapter_increment.add_argument("--output", required=True, help="?? JSON ????")
    ...
    convergence_matrix.add_argument("--candidate", required=True, help="??????")
    convergence_matrix.add_argument(
        "--reference",
        help="Default navdata 2608R1 ??????????????",
    )
    convergence_matrix.add_argument(
        "--repeat-candidate",
        help="??????????",
    )
    convergence_matrix.add_argument("--output", required=True, help="?? JSON ????")
    ...
    source_gap.add_argument(
        "--table",
        action="append",
        choices=("waypoint", "airway"),
        help="可选的单表来源探针；重复传入可同时选择 waypoint 和 airway",
    )
    ...
    pipe_master = sub.add_parser(
        "pipeline-master-audit",
        help="只读运行 424 来源-模型-BGL 端到端全量流水线主审计门禁",
    )
    pipe_master.add_argument(
        "--raw-root",
        required=True,
    ...
    airline_source = sub.add_parser(
        ...
        help="只读复核 424 航司航线网络与系统配置元数据关系",
    )
    airline_source.add_argument(
        "--raw-root",
        required=True,
        help="2608 原始 CSV/PDF 目录",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "runtime-contract-audit":
        ...
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0
    if args.command == "adapter-increment-authorization-audit":
        output = Path(args.output).expanduser().resolve()
        report = audit_adapter_increment_authorization()
        report["output"] = str(output)
        write_adapter_increment_authorization_audit(output, report)
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0
    ...
    if args.command == "convergence-decision-matrix-audit":
        reference = _path(args.reference) or detect_paths().reference_root
        if not reference:
            raise SystemExit("?????? Default navdata 2608R1 ?????????? --reference")
        output = Path(args.output).expanduser().resolve()
        report = audit_convergence_decision_matrix(
            Path(args.candidate),
            reference,
            repeat_candidate_root=_path(args.repeat_candidate),
        )
        report["output"] = str(output)
        write_convergence_decision_matrix(output, report)
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0
    ...
    if args.command == "source-model-completeness-audit":
        ...
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0
    if args.command == "pipeline-master-audit":
        report = audit_pipeline_master(
            Path(args.raw_root),
            load_model(Path(args.model)),
        )
        output = Path(args.output).expanduser().resolve()
        report["output"] = str(output)
        write_pipeline_master_audit(output, report)
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0 if report["summary"]["pipeline_master_verified"] else 1
    ...
    if args.command == "airline-system-source-audit":
        report = audit_airline_system_source(
            Path(args.raw_root),
            load_model(Path(args.model)),
        )
        output = Path(args.output).expanduser().resolve()
        report["output"] = str(output)
        write_airline_system_source_audit(output, report)
        print(json.dumps(report, ensure_ascii=False, indent=2, default=str))
        return 0
