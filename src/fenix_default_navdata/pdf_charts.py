...
_STANDARD_TABLE_LABEL = re.compile(r"^[A-Z][A-Z0-9]{1,5}-(?:\d{1,2}[A-Z]{1,2}|[A-Z]{1,2}\d{1,2}|\d{1,2})$")
_STANDARD_TABLE_ROUTE = re.compile(r"^[A-Z][A-Z0-9]{1,5}(?:-[A-Z][A-Z0-9]{1,5})+$")
    ...
    for localizer in matches:
        ...
        if glide_path is not None:
            ...
            angle_match = re.search(
                r"(?:(?P<angle>\d(?:\.\d+)?)\s*[°º]\s*(?:下滑角|GP)|"
                r"(?:下滑角|GP)\s*(?P<labeled_angle>\d(?:\.\d+)?)\s*[°º])",
                glide_path["tail"], re.IGNORECASE,
            )
            glide_angle = float(angle_match["angle"] or angle_match["labeled_angle"]) if angle_match else None
...
def _wrapped_standard_procedure_routes(
    words: list[tuple[float, float, float, float, str, int, int, int]],
) -> tuple[ChartStandardProcedureRoute, ...]:
    """Read a wrapped right-hand route cell without changing direct-table evidence."""
    headers = {
        text.strip(): (x0, y0)
        for x0, y0, _, _, text, *_ in words
        if text.strip() in {"进场程序代号", "离场程序代号", "导航数据代号", "航迹简述"}
    }
    label_header = headers.get("进场程序代号") or headers.get("离场程序代号")
    code_header, route_header = headers.get("导航数据代号"), headers.get("航迹简述")
    if label_header is None or code_header is None or route_header is None:
        return ()
    label_x, header_y = label_header
    code_x, _ = code_header
    route_x, _ = route_header
    rows = []
    for x0, y0, _, _, text, *_ in words:
        label = text.strip().upper()
        if not (y0 > header_y and label_x - 12 <= x0 < code_x - 6 and _STANDARD_TABLE_LABEL.fullmatch(label)):
            continue
        codes = [
            raw.strip().upper()
            for code_x0, code_y0, _, _, raw, *_ in words
            if code_x - 12 <= code_x0 < route_x - 6
            and abs(code_y0 - y0) <= 3
            and re.fullmatch(r"[A-Z][A-Z0-9]{2,5}", raw.strip().upper())
        ]
        if len(codes) == 1:
            rows.append((y0, label, codes[0]))
    routes = []
    for index, (row_y, label, code) in enumerate(rows):
        lower = float("-inf") if index == 0 else (rows[index - 1][0] + row_y) / 2
        upper = (
            float("inf") if len(rows) == 1 else row_y + (row_y - rows[index - 1][0]) / 2
            if index + 1 == len(rows) else (row_y + rows[index + 1][0]) / 2
        )
        route = "".join(
            raw.strip().upper()
            for _, _, raw in sorted(
                (word_y0, word_x0, raw)
                for word_x0, word_y0, _, _, raw, *_ in words
                if route_x - 50 <= word_x0 and header_y < word_y0 and lower <= word_y0 < upper and raw.strip()
            )
        )
        fixes = tuple(route.split("-"))
        version = label.rsplit("-", 1)[1]
        if _STANDARD_TABLE_ROUTE.fullmatch(route) and len(fixes) >= 2 and fixes[0] == label.split("-", 1)[0] and code.endswith(version):
            routes.append(ChartStandardProcedureRoute(label, code, fixes))
    return tuple(dict.fromkeys(routes))
...
def _normalize_approach_family(value: str) -> str:
    normalized = re.sub(r"\s+", " ", value or "").strip().upper()
    if "ILS" in normalized:
        return "ILS"
    if "RNP" in normalized:
        return "RNP_AR" if "AR" in normalized else "RNP"
    return ""
    ...
    def flush() -> None:
        ...
        result.extend(
            ChartTerminalLeg(
                ...
                active_transition,
                center_ident,
                approach_family=active_approach_family,
            )
    ...
    for line_number, line in enumerate(lines):
        ...
        if heading or approach_heading:
            ...
            if approach_heading:
                ...
                active_approach_family = _normalize_approach_family(
                    approach_heading.groupdict().get("family") or ""
                )
                if ar_variants and not active_approach_family:
                    active_approach_family = "RNP_AR"
                label_prefix = "I" if active_approach_family == "ILS" else "R"
        ...
        for index, leg in enumerate(legs):
            ...
            elif leg_type in {"CF", "CA"}:
                # PDF table order can place the tail of an unrelated column
                # before the next procedure heading.  Only CF/CA rows have
                # the documented leading-row relationship with that heading;
                # RF/TF/DF/IF rows must not be guessed into the next SID/STAR.
                pending_rows = [row]
            else:
                pending_rows = []
    flush()
...
def extract_approach_chart(pdf: Path, airport: str, chart_type: str, chart_name: str, *, include_vector_evidence: bool = False) -> list[ProcedureChart]:
    ...
    with pymupdf.open(pdf) as document:
        for page_number, page in enumerate(document, start=1):
            ...
            table_standard_routes = _wrapped_standard_procedure_routes(words) if chart_type == "standard-terminal-procedure" else ()
            result.append(replace(chart, route_fixes=tuple(dict.fromkeys(route_fixes)), standard_routes=standard_routes, table_standard_routes=table_standard_routes))
    return result
