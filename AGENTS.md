...
| Git/远端 | 本地待同步 | `main` 当前领先 `origin/main` 74 个提交；r253 报告被忽略，尚未提交；此前普通推送因 `127.0.0.1:7897` 未监听失败 | 每次提交后只尝试普通 `git push`；网络恢复后用 `git ls-remote --heads origin main` 复核，禁止强推 |
