...
- r390：新增可复用 `package-derived-metadata-audit`。manifest/ContentHistory 数量等项目定义差异可记录，但 layout 日期、`bglIndex.bout`、BGL 尺寸和日期关联依赖内容/生成过程；禁止复制参考元数据或反推 payload。两包结论均为 `unexplained_without_content_inference`，未授权修改。证据：`diagnostics/r390-package-derived-metadata-audit.json`。
