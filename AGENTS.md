...
- **R434 终端点去重与机场 BGL 最小探针（进行中，2026-08-25）：** 当前源码生成的十个机场 XML 跨机场 `(waypointRegion, waypointIdent)` 重复数均为 0，修复确认有效；以冻结 `intermediate-2608-r394-global-procedure-ils-fix.json.gz` 生成的 r434 候选仍因 Package Tool 返回 1 被硬门禁拒绝。独立 ZUUU 探针在“完整机场/去 Holding/去程序与航点/去 DeleteAirport”多种矩阵下均在读取器报 `record type 0x0`，机场/跑道表为空但 ILS/航点残留，说明剩余阻塞是机场基础记录的 SDK 编译表达或 Package Tool 产物完整性，不是 424 程序来源缺失。r434 不得部署。证据：`output/candidate-2608-default-r434-terminal-global-dedup`、`diagnostics/r434-zuuu-probe-*`、`F:\games\community\_work\r434-zuuu-reader-*`。
- **R435 运行时包边界修复（已完成暂存，2026-08-27）：** 发现 Community 旧 r404 主包错误地重复包含十个区域机场 BGL，而机场补丁包也包含同一批 BGL；新增 `runtime_package_audit` 内容门禁，强制主包只含 `00_enroute.bgl`、补丁包只含十个区域机场 BGL。使用完整 `candidate-2608-default-r433-package-split` 进行功能测试暂存，备份为 `F:\games\community\backups\default_navdata_20260827_225738`；官方两个包原样保留，机场补丁包未变，主包重复机场 BGL 已移除。部署后包审计 `valid=true`，转换器全量 `538 passed`。该阶段验证重点为 ZUUU 跑道/进近是否因重复机场覆盖消失；r434 仍不得部署。
- **R436 ZUGY 离场分段与全局航点去重（代码完成，候选构建阻塞，2026-08-28）：**
  1. 修复离场程序跑道过渡与公共段分段解析（`pdf_charts.py`）：新增 `_DATABASE_RUNWAY_TRANSITION`（`RWY...跑道过渡`）、`_DATABASE_COMMON_SECTION`（`公共段/公用段`）及 `_DATABASE_ENROUTE_TRANSITION`（`...航路过渡`）匹配分支。解决了此前将多跑道过渡航段（如 01L 与 01R）与公共航段串联至同一离场程序导致的 ZUGY 离场航点位置错乱与往返折线问题。
  2. 修复机场局部对全局航点的重复遮蔽（`bgl.py`）：在 `write_bglcomp_xml` 机场及根级终端航点输出中增加 `_waypoint_matches_terminal_point` 过滤，并在 `_resolve_source_fix` 中优先解析全局航点；避免如 `ABVIL`、`ESLIR` 等 5 字全局航路点被局部机场 BGL（如 ZSHC/ZUGY）错误发布为无航路属性的私有终端航点而导致机模 FMC 报 `WAYPOINT NOT ON AIRWAY`。
