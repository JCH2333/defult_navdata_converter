...
| 自动化回归 | 已通过 | r248 后 `454 passed in 4.01s` | 仅证明已编码约束，不证明字节一致、部署或实机可用 |
| 部署、恢复、实机、Release | `0%` | 当前 `deployable=false`；本轮确认 `FlightSimulator2024.exe` 未运行 | 参考 `29/29`、干净复建、恢复演练和用户实机清单全部通过 |
| Git/远端 | 本地待同步 | 工作树干净；`main` 比 `origin/main` 领先 68 个提交；代理 `127.0.0.1:7897` 未监听 | 每次提交后普通推送；网络恢复后用 `git ls-remote --heads origin main` 复核，禁止强推 |
