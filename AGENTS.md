...
## 2026-08-19 r241 权威项目状态、进度统计与可复用收敛计划

本节自 r241 起是默认通用数据项目唯一的当前状态入口，优先于本文件此前的“当前状态”“下一项任务”和进度估算。历史章节仍是可复核实验记录。每次续做前，Codex 必须实际核对两份 `AGENTS.md`、`git status --short --branch`、冻结模型 SHA-256、r188/r189 的 `conversion-report.json`、最新来源卡诊断、完整测试、候选/参考收敛审计和 `FlightSimulator2024.exe` 进程；文字与产物冲突时，以可复跑产物为准，并在同一轮更新本节及工作区根说明。

### 1. 本轮实际核对结果

| 轨道 | 当前事实 | 结论 |
| --- | --- | --- |
| 内容边界 | 中国内容只来自 `424源数据\2608\2608` 的 CSV/PDF；`navigraph-nav-base`、`navigraph-nav-jepp` 只作官方模板和加载契约；`Default navdata 2608R1` 只读验收 | 禁止 Fenix、参考 BGL/SQLite、参考坐标、哈希或派生元数据进入内容投影 |
| 冻结模型 | `output\intermediate-2608-r187-navaid-label-replay.json.gz`，SHA-256 `7cec24bd4a57545d39aab037abe4125c763ad12f364bd5f8f0073b0e050fdb4b` | 所有可接入内容规则必须先生成新模型并通过模型重放 |
| 最新候选 | r188/r189 均为 `test_build=true`、`local_contract_verified=true`、`deployable=false` | 同一冻结模型独立构建的受控文件树自重放为 `29/29` |
| 字节验收 | 参考受控范围为 29 个文件，当前 SHA-256 一致数为 `0/29` | 构建确定性已经证实，参考字节收敛尚未开始达标 |
| 来源缺口 | 已闭合结论 32/40；剩余 8 张均为 IAP 主段来源不足。r238 至 r240 已对 `ZSNJ:I25`、`ZSOF:R33`、`ZSWY:I03` 建立精确来源卡，三者仍只能保守拒绝 | “存在精确审计”不等于“内容已闭合”或“允许投影” |
| 自动化 | 最近完整回归基线为 `442 passed` | 回归仅证明已编码约束；不证明参考一致、游戏加载或可部署 |
| 部署与发布 | 未覆盖 `F:\games\community\Community`，未为当前候选建立部署备份，未做恢复演练和实机验证，未创建 Release | `deployable=false` 时 GUI、CLI、自动更新和部署器均不得写入 Community |
| Git 与网络 | 工作树干净；`main` 比 `origin/main` 领先 57 个提交；远端为 `JCH2333/defult_navdata_converter`；`127.0.0.1:7897` 当前未监听 | 保留本地提交。每次仓库改动后仅尝试普通 `git push`；网络恢复后以 `git ls-remote --heads origin main` 复核，禁止强推 |

工程基础能力仍可粗略估为约 `45%`，只包括来源解析、证据审计、版本化模型、默认 BGL 适配器、CLI/GUI、ASCII Package Tool 构建、验证、更新和部署门禁。字节验收必须单独记为 `0/29`，部署、实机和正式 Release 均为 `0%`；不得将这三类指标合并为上线进度。

### 2. 已确认经验和不可回退边界

1. `NavModel` 是跨 AIRAC、跨目标格式唯一的内容边界。新适配器只能消费版本化模型，不能在 adapter 内重新解释 CSV/PDF，更不能读取 Fenix 或参考成品。
2. 审计结论必须区分“可投影的唯一规则”“有证据的拒绝”和“来源不足未决”。只有第一类可以进入模型或 adapter；第二类和第三类都是防止错误投影的长期资产。
3. OCR 只可在已有模型对象、精确 `SourceRef`、原始 PDF SHA-256、页面归属和可重复运行都匹配时做受控消歧。OCR 不能生成主进近、程序腿、坐标、类型、地区或图页所有权。
4. 未分类程序 13 张卡已经全部因缺少标签到类别的直接锚点而拒绝；航路端点和派生全局航点 17 张卡已因多地区、ACC 不完整/冲突或身份不足而拒绝。不得用跑道、腿标识、邻近标题、节表、参考差异或 OCR 重新包装为可投影规则。
5. 机场关联 VOR/NDB、机场扇区无线电、跑道表面/阈值猜测、根终端点重复、等待航线隔离、`CODE_DIR` 简单反转、无来源 `routeType` 与移除 `DeleteAirport` 均已否决。重新立项必须同时提供新的同周期直接来源、单变量 SDK/运行时证据和最小反例。
6. BGL 节表、包大小、`bglIndex.bout`、`layout.json`、`manifest.json`、ContentHistory 和参考哈希只能定位或验收，不能作为内容输入、手工修补对象或复制到候选。

### 3. 按顺序执行的详细计划

#### 阶段 0：每轮基线和单变量登记

每轮分配连续 `rNNN`，只能有一个可证伪假设和一个变量。开始前记录输入、工具和缓存哈希，允许/禁止来源，预期对象、目标文件角色、成功条件和否决条件。结束后必须以实际命令更新模型哈希、测试数、来源卡状态、候选状态、自重放、参考 `x/29`、部署状态、Git 领先数、经验和下一项唯一任务。

#### 阶段 A：完成 IAP 主段来源队列

下一项唯一任务为 `ZUAL:I15` 精确 IAP 主段来源卡审计。先从冻结模型锁定其精确数据库页，分别输出主进近、过渡和复飞腿，再将每张同跑道候选仪表图的标题、直接角色、固定点、PDF SHA-256 和缓存来源逐项绑定。

后续 IAP 卡按稳定排序逐张处理。只有“唯一数据库主段 + 唯一同周期图页归属 + 直接角色或固定点可重放关联”同时成立，才允许提出一条候选投影规则；先补最小正反 fixture 和审计字段，再进入阶段 B。缺少任一条件即保留 `unresolved_direct_database_evidence_inconclusive` 或精确拒绝，禁止从同跑道、标题相似、重叠 IAF、复飞、页面顺序或 OCR 拼接主进近。

阶段出口是 8 张 IAP 卡均有可复跑的唯一规则或精确拒绝结论，不是强行补齐全部参考内容。

#### 阶段 B：来源规则接入和模型重放

只接入阶段 A 中 `projection_allowed=true` 且已有目标表达证据的规则。固定顺序为：

`最小正反 fixture -> 来源审计字段 -> 规则实现 -> 新版本模型 -> model-replay-audit --fail-on-unexpected -> 模型语义差异白名单 -> 候选构建`

白名单必须逐项保存对象身份、字段路径、源 `SourceRef`、预期 BGL 文件角色和候选差异。出现未允许模型差异、引用断裂、降级计数异常或跨范围 BGL 影响时，立即停止并回退到只读诊断，不得继续构建字节候选。

#### 阶段 C：目标格式和 SDK 表达能力取证

若来源队列没有可接入规则，转为最小 SDK/加载契约探针，不能扩大 424 猜测。每个探针只修改一项 XML 表达，必须保留控制组与变量组 XML、输入和工具 SHA-256、Package Tool 进程轨迹、完整输出树哈希、BGL 头/节表、读取器完整登记、对象计数和可证伪结论。

探针优先级为：`00_enroute.bgl` 的已有来源对象表达，区域机场 BGL，机场补丁 BGL，最后才是索引和派生元数据。节表差异只能定义下一项探针的测量指标，不能推断对象含义或生成内容。

#### 阶段 D：候选双构建和字节收敛

每次有效模型、adapter、SDK、模板或构建策略变化，都必须从相同冻结模型、官方双基线、ASCII 暂存策略和输入 manifest 独立构建两次，固定执行：

`validate -> 标准 JSON 重读 -> 29 文件自重放 -> 来源缺口审计 -> BGL/layout/cardinality 审计 -> file-convergence-audit`

按航路 BGL、区域机场 BGL、机场补丁 BGL、索引、布局、清单和 ContentHistory 分组记录差异。只有模型重放无未允许差异、自重放为 `29/29`、影响文件角色符合假设且参考一致文件数增加，才能标记“字节收敛推进”；其余一律标记“字节收敛未推进”。

#### 阶段 E：干净验收、备份、部署和实机

仅在参考 `29/29`、全新隔离目录双构建一致、完整验证和来源审计通过后执行：

1. 再次确认 `FlightSimulator2024.exe` 已退出。
2. 对两个 Community 覆盖包及 `layout.json`、`manifest.json`、`bglIndex.bout`、ContentHistory 创建带时间戳和 SHA-256 清单的完整备份。
3. 先执行恢复演练并验证备份树哈希，再覆盖 `F:\games\community\Community`。
4. 由用户实机验证 `ZBCF`、`ZUNZ`、`ZUUU` 的机场输入、跑道、SID、STAR、IAP、航路/航点、退出飞行和退出模拟器。

实机通过前仅可标记测试版；正式 GitHub Release 只能在全部清单通过且普通推送成功后创建。
...
1. 每个 AIRAC 建立输入 manifest：CSV/PDF、官方模板、SDK、解析器版本和 OCR 运行时指纹均记录 SHA-256。周期、版本或哈希不匹配时拒绝混用。
2. `NavModel` 保持格式化版本号、原始精度、单位、`SourceRef`、引用关系和显式降级计数；模型读取器必须向后兼容或提供可审计迁移，不允许静默改变旧字段语义。
3. 新目标格式必须先创建独立 `profile/adapter/validator/deployer` 说明与最小 fixture，登记官方基线、真实加载路径、文件/schema、字段单位、NULL/default、物理排序、字符串/容量限制、元数据、运行时模拟器、不可表达项降级、GUI/CLI 接入和实机清单。
4. `project-target` 只能消费模型，不得把目标专有分支写回 `source.py`。通用来源规则归属解析/模型层；目标格式规则归属独立 adapter；共享验证器只保留与目标无关的完整性门禁。
5. GUI、CLI、自动更新、候选报告和部署必须共用同一 profile 和 `deployable` 门禁。自动更新只更新已签名或有 SHA-256 的工具发行物，不能下载或覆盖导航内容资产。
6. 每个新格式至少建立：来源边界测试、字段映射 fixture、格式完整性验证、确定性双构建、部署备份恢复演练和实机清单。默认通用数据的未证实 SDK 规则不得直接迁移。
...
## 2026-08-19 r243 SDK 表达能力库存盘点

- 实验编号：`r243-sdk-expression-capability-inventory`。本轮使用现有可复用 CLI `airport-source-inventory`，只读取 r187 冻结模型和 r188 的候选 `china-navdata.xml`，并以 r194/r195/r215、r214 与 r216 的既有只读结论判断探针历史；不读取参考 BGL payload、参考导航记录、Fenix 或 OCR，不修改模型、投影、候选、Community 或部署。
- 真实报告为 `diagnostics\r243-sdk-expression-capability-inventory-20260819.json`。来源模型有机场 `275`、跑道方向 `640`、终端航点 `12549`、ILS `430`、VOR `361`、NDB `77`；候选 XML 有 `Airport=275`、`Runway=320`、`Waypoint=15699`、`Route=5788`、`Previous=4434`、`Next=4434`、`Vor=121`、`Ndb=133`。该清单只说明当前投影规模，不能和任何 BGL 节类型建立对象等价关系。
- 机场侧唯一标记为 `eligible_for_sdk_probe` 的来源完整类别是 `RWY_DIRECTION.VAL_THR_DISPLACE -> OffsetThreshold`，共有 33 条记录；但 r195 已以来源完整的最小探针证明阈值位移不产生候选缺失的 `0x17/0x33` 节，不能重复作为字节修复方向。跑道表面已由 r194 证伪，`DeleteAirport` 已由 r215 证伪为节表基数来源；机场关联导航台和进近扇区频率分别受来源作用域与基数约束拒绝。
- 航路侧 r216 仍仅提供定位量：`00_enroute.bgl` 的 `0x13=-12`、`0x17=-4`、`0x22=-419`，总大小 `-43,820` 字节。端点地区来源卡和未分类程序卡均已完成保守拒绝，IAP 主段卡也没有新的可投影规则。因此当前没有一个能从机场库存直接升级为正式 adapter 变更的候选；字节收敛未推进。
- 可复用结论：SDK 探针选择必须同时满足“当前 424 有直接来源、对象作用域与目标文件角色明确、未被历史单变量实验否决”。库存中只有第一项为真而后两项缺失的类别，必须继续保持诊断性候选，不能因字段看似可表达而修改正式投影。
- 下一项唯一任务：启动 r244 航路序列化投影矩阵审计。它只读取 r187 与 r188 的 `china-navdata.xml`，按每条 `AirwayLeg` 的来源身份、航路名、序号、端点、地区、`Route`/`Previous`/`Next` 输出形状和 XML 位置建立可重放关联，专门识别“模型已投影”“来源拒绝”“模型/输出不一致”三类，而不读取参考 payload 或把 r216 节表差值映射为对象。审计完成前不得再次运行机场 SDK 子对象探针。

## 2026-08-19 r244 权威进度口径、收敛计划与复用管线维护

本节是 r241 的执行性补充。每次继续默认通用数据工作前，Codex 必须先核对本节和 r241，再以可复跑产物更新状态；禁止仅按历史文字、候选目录名或累计提交数报告进度。

### 1. 本次核对的实际状态

| 维度 | 实际状态 | 进度口径 |
| --- | --- | --- |
| 内容来源与输入冻结 | 仅允许 `424源数据\2608\2608` CSV/PDF；冻结模型 r187 SHA-256 `7cec24bd4a57545d39aab037abe4125c763ad12f364bd5f8f0073b0e050fdb4b` | 已建立，后续规则必须生成新模型并做严格模型重放 |
| 可复用工程能力 | 424 读取、证据审计、`NavModel` 序列化、默认 BGL adapter、CLI/GUI、SDK ASCII 暂存构建、验证、更新和部署门禁均已实现 | 约 45%，只表示工程基础，不表示交付或可部署 |
| 构建确定性 | r188/r189 受控文件树自重放 `29/29`；`test_build=true`、`local_contract_verified=true` | 已达标；任何修改后必须保持 |
| 参考字节验收 | 两个中国覆盖包受控范围共 29 文件，与 `Default navdata 2608R1` 一致 `0/29` | 0%，是当前主阻塞；不得与构建确定性混为一项 |
| 来源缺口 | 40 张卡中 32 张得到可复核的闭合结论；余下 8 张是缺少唯一主进近的 IAP 卡，仍不允许投影 | 审计覆盖不等于内容可写入 |
| 自动化 | 最近完整回归基线 `442 passed` | 已通过的约束回归；不等于字节一致、游戏加载或实机可用 |
| 部署和实机 | 未覆盖 Community，未对当前候选做恢复演练，未实机验证，未发布 | 0%；`deployable=false` 时不得部署或创建 Release |
| Git 和网络 | 本计划提交前工作树干净，`main` 比 `origin/main` 领先 60 个提交；`127.0.0.1:7897` 未监听，远端查询失败 | 保留本地提交；每次改动后只尝试普通推送，网络恢复后用 `git ls-remote --heads origin main` 复核 |

### 2. 进度更新协议

每个后续 `rNNN` 必须在同一轮结束时同时更新仓库 `AGENTS.md` 与工作区根 `AGENTS.md`，至少记录以下字段：

1. 唯一假设、唯一变量、允许读取的来源和明确禁止读取的内容。
2. 输入模型版本及 SHA-256、候选版本、工具/SDK 版本和必要输入 manifest。
3. 新增或复用的可自动化审计、最小正反 fixture、完整测试结果和 `git diff --check` 结果。
4. 模型差异、来源拒绝/降级计数、候选双构建自重放、29 文件参考一致数，以及受影响文件角色。
5. 可证伪结论：允许进入正式 adapter、仅保留为诊断，或明确拒绝；不得只记录“尝试过”。
6. Git 提交哈希和普通推送结果。推送失败要记录失败原因，但不得重写历史或强推。

进度必须始终拆成四条互不替代的轨道：工程基础能力、来源规则闭合、参考字节一致数、部署/实机/发布。只有参考一致数真实增加，才能写“字节收敛推进”；只有完成部署和用户实机清单，才能写“可上线测试”。

### 3. 从 r244 开始的详细执行计划

#### 阶段 A：航路投影矩阵和正式差异归因

1. 执行 r244 的只读矩阵审计。逐条关联 `NavModel.airway_legs` 与 r188 XML 的 `Route`、`Previous`、`Next`，稳定输出来源身份、航路名、序号、起终点、地区、XML 路径和相邻连接形状。
2. 每条航路腿只能分类为：`projected`、`rejected_by_source`、`missing_from_xml`、`ambiguous_output_match` 或 `invalid_source`。报告必须可重放且分类总数守恒。
3. 若出现 `missing_from_xml` 或确定性的 `ambiguous_output_match`，先为该精确序列化规则写最小正反 fixture；只有来源完整、目标 XML 表达明确且未被反证时，才允许修改 adapter。
4. 若矩阵显示模型与 XML 一致，则把 r216 的航路 BGL 节表差异保持为格式/SDK 问题，不得从参考节表反推出内容，也不得继续枚举机场子对象。

阶段 A 的退出条件是：产生一份完整矩阵报告，所有航路腿都有稳定分类，且下一项实验被收敛为一个可证伪的 XML 表达假设或“当前无可接入来源规则”。

#### 阶段 B：单变量 SDK 和加载契约探针

1. 仅在阶段 A 或新的来源审计提供可接入对象后启动探针。每次探针只修改一项 XML 表达，使用控制组和变量组，禁止同时混入模型修改。
2. 固定保存输入 XML、输入树哈希、SDK/Package Tool 路径与版本、启动/等待进程证据、完整输出树哈希、BGL 头/节表、读取器完整登记和结论 JSON。
3. 探针结论必须区分“某表达可编译”“某表达改变 BGL 布局”“某表达满足加载契约”。前两者不能自动推出后者。
4. 被否决的对象类别登记为不可重复探针；只有取得新的独立 424 语义、不同目标作用域或不同 SDK 契约证据，才允许重新打开。

阶段 B 的退出条件是：形成一个被来源和目标表达同时支持的 adapter 变更，或明确没有剩余未重复的单变量探针。

#### 阶段 C：模型和 adapter 的受控接入

1. 顺序固定为：最小 fixture -> 来源审计字段 -> 规则实现 -> 新模型版本 -> `model-replay-audit --fail-on-unexpected` -> 模型语义差异白名单 -> adapter 投影。
2. 白名单逐项写明对象身份、字段路径、`SourceRef`、预期 BGL 文件角色和预期差异；任何跨范围差异、引用断裂、未知 NULL/default、字符串截断或降级计数增加都必须停止并回到只读诊断。
3. 不得修改冻结 r187。每个可接入规则生成新的版本化 `NavModel`，并保留输入 manifest 与模型 SHA-256，供其他目标格式直接消费。
4. OCR 只能在既有对象、既有 PDF 和精确 `SourceRef` 上做可重放消歧；它不得创建机场、导航台、坐标、程序主段、程序腿、类别或图页归属。

阶段 C 的退出条件是：模型重放无未允许差异，目标 validator 通过，且变更范围与该轮假设一致。

#### 阶段 D：双构建、文件收敛与参考验收

每个有效变更必须从同一模型、同一官方双基线、同一 SDK、同一 ASCII 暂存策略和同一输入 manifest 独立构建两次，并依次执行：

`validate -> 标准 JSON 重读 -> 29 文件自重放 -> source-gap audit -> BGL/layout/cardinality audit -> file-convergence-audit`

按 `00_enroute.bgl`、区域机场 BGL、机场补丁 BGL、`bglIndex.bout`、`layout.json`、`manifest.json`、`ContentHistory.json` 分组报告。只有同时满足以下全部条件，才允许更新“字节收敛推进”：
...
若一致数不增加，记录为“字节收敛未推进”，保留报告和结论，不得以重命名候选、重写元数据或手工修改 BGL 掩盖差异。

#### 阶段 E：干净验收、恢复演练、部署和用户实机

只有参考 `29/29`、从全新隔离目录双构建一致、完整 validator 和来源审计通过后，才可进入部署准备：

1. 确认 `FlightSimulator2024.exe` 已退出。
2. 对 Community 中两个覆盖包、`bglIndex.bout`、布局、清单和 ContentHistory 建立带时间戳的完整备份及 SHA-256 清单。
3. 先在非 Community 目标完成一次恢复演练，并验证恢复树哈希。
4. 通过 CLI/GUI 同一部署器覆盖 `F:\games\community\Community`，部署后立即核对输出树 SHA-256 和完整 validator。
5. 交由用户在 `ZBCF`、`ZUNZ`、`ZUUU` 验证机场输入、跑道、SID、STAR、IAP、航路/航点、退出飞行和退出模拟器。

用户实机通过前产物只能标记为测试版，自动更新仅可分发测试包；正式 GitHub Release 仍然禁止。

### 4. 面向后续 424 周期和其他格式的可复用管线

长期固定管线为：

`lock-inputs -> ingest-424 -> evidence-audit -> normalize-model -> model-audit -> project-target -> build-target -> validate-target -> diff-and-audit -> stage-backup-deploy`

1. `lock-inputs`：登记 AIRAC、原始 CSV/PDF 树哈希、官方模板树哈希、SDK/工具版本和周期元数据。
2. `ingest-424` 与 `evidence-audit`：只从 424 建立结构化事实及 `SourceRef`；OCR 缓存是有版本和哈希绑定的辅助证据，不能替代原始来源。
3. `normalize-model`：输出可版本化、可序列化、可重放的 `NavModel`；模型持有原始精度、单位、来源、拒绝和降级原因，不能掺入目标格式字段。
4. `project-target`：每个机模独立实现 `profile/adapter/validator/deployer`，登记 schema/格式、加载契约、字段与单位、NULL/default、排序、字符串限制、元数据、不可表达策略和最小 fixture。
5. `build-target` 与 `validate-target`：构建器必须确定性；验证器包含完整性、元数据、引用、真实 SQL/加载器模拟器（已取得时）、文件布局和目标专用边界检查。
6. `diff-and-audit`：参考产物只能读作验收信号，差异报告必须避免导出可反向填充的参考内容；以来源闭合和可证伪探针决定下一步。
...
- 完整回归为 `446 passed`，`git diff --check` 通过。代码和测试提交为 `80c70e4`；普通 `git push` 仍因 `127.0.0.1:7897` 未监听而失败，未强推。
...
## 2026-08-19 r245 SDK BGL 格式表达能力矩阵
...
## r249 权威状态、进度口径与后续执行计划（2026-08-19）

本节是后续默认通用数据工作的权威入口。历史章节保留单轮证据；文字与可复跑产物冲突时，以
实际命令输出、模型哈希、诊断报告、测试结果和 Git 状态为准，并在同一轮同步工作区根
`AGENTS.md`。
...
| 轨道 | 当前状态 | 量化依据 | 阶段出口 |
| --- | --- | --- | --- |
| 内容边界 | 已锁定 | 仅 `424源数据\2608\2608` CSV/PDF 可形成内容；r187 `NavModel` SHA-256 为 `7cec24bd4a57545d39aab037abe4125c763ad12f364bd5f8f0073b0e050fdb4b` | 任何新模型都保留原始精度、单位、`SourceRef`、引用、拒绝和降级记录 |
| 工程与复用管线 | 约 `45%` | 已有版本化模型、默认 BGL adapter、CLI/GUI、Package Tool、validator、审计、更新和部署门禁 | 新目标格式可复用模型，并各自建立 profile/adapter/validator/deployer |
| 来源缺口闭合 | `32/40` | r236：航路端点 12、航路点 5、IAP 10、未分类程序 13；其中 8 张 IAP 主进近卡仍不可投影 | 每张卡均有可重放允许规则或精确拒绝结论；拒绝不计为新增内容 |
| 候选确定性 | 稳定基线 `29/29` | r188/r189 同冻结模型、同双基线、同 SDK、同 ASCII 暂存策略逐文件相同 | 新模型或工具链变量均须重新完成独立双构建 |
| r248 元数据探针 | 已完成并否决为收敛手段 | 保留 FILETIME 相对 r188 为 `25/29`；仅两个包的 `layout.json` 与 `bglIndex.bout` 改变 | 默认时间正规化继续作为确定性门禁，禁止手工回写 FILETIME |
| 参考字节验收 | `0/29` | `r248-file-convergence-20260819.json`：29 个受控文件全部不等于参考 | 全新隔离目录双构建均达到参考 `29/29` |
| SDK/模板变量 | r249 初检完成 | SDK `1.6.9`；`fspackagetool.exe` SHA-256 `EDA75A7D187FB45EDE71B59DD3189E614880F44642294600AA1AEBDD781A6F51`；本机未发现第二份 Package Tool | 仅在发现具体、可哈希、可恢复的替代版本后启动单变量探针 |
| 自动化回归 | 已通过 | 本轮计划更新后的两次完整回归均为 `454 passed` | 仅证明已编码约束，不证明字节一致、部署或实机可用 |
| 部署、恢复、实机、Release | `0%` | 当前 `deployable=false`；本轮确认 `FlightSimulator2024.exe` 未运行 | 参考 `29/29`、干净复建、恢复演练和用户实机清单全部通过 |
| Git/远端 | 本地待同步 | r249 计划提交 `e0ed8f9` 后普通 `git push` 失败：`127.0.0.1:7897` 未监听；未强推、未重写历史 | 每次提交后普通推送；网络恢复后用 `git ls-remote --heads origin main` 复核，禁止强推 |

工程能力、来源证据、候选确定性、参考字节验收和部署实机是相互独立的轨道，禁止相加成单一
“完成百分比”。当前可以表述为“可复用转换与验证管线已建立，来源和编译收敛处于中期”，
不能表述为“接近上线”。除非参考一致数量实际增加，不得用测试数、文件大小、节表或候选
自重放替代字节收敛。

### 2. 已确认经验与冻结项

1. `NavModel` 是唯一跨 AIRAC、跨格式的内容边界。新机模只能消费版本化快照，不能重新解析
   424、读取 Fenix，或把 OCR 缓存当内容来源。
2. 官方 `navigraph-nav-base`/`navigraph-nav-jepp` 只提供全球基线和加载契约；
   `Default navdata 2608R1` 仅作只读验收。参考 BGL/SQLite、坐标、哈希、节表、包元数据和
   索引不得反向补写导航对象、XML 或元数据。
3. OCR/本地多模态模型只能对已有对象的精确 `SourceRef` 做受控消歧，必须记录 PDF SHA-256、
   页码、运行时、参数、重复结果和正反例；不得创建机场、导航台、坐标、程序主段、腿类别或
   图页归属。
4. r244 已闭合航路模型到 XML 的序列化。r246 已否决跑道表面、阈值位移和 `DeleteAirport`
   作为当前机场 BGL 差异的修复方向。除非获得独立 424 来源或真实 SDK/运行时反证，不得重跑。
5. r247/r248 已证明 Package Tool FILETIME 只影响派生索引/布局，不会提升参考一致性。禁止
   手工回写 FILETIME、包大小、ContentHistory、layout、manifest 或索引二进制。
6. 代码、来源规则、目标适配、SDK 探针、候选构建和部署逻辑必须分主题提交。数据库、备份、
   日志、诊断、模型快照、OCR 缓存、SDK 中间产物和外部测试包不入库。

### 3. r250 起的分阶段计划

#### 阶段 A：完成 SDK/模板可用性判定

目标是判断是否存在本机可控的构建环境变量，而不是猜测导航内容。

1. 只读记录 SDK、Package Tool、根项目 XML、PackageDefinitions、官方 manifest 非导航字段、
   输入 manifest 和 ASCII 暂存策略。
2. 仅检查本机已安装或已缓存的其他 SDK/Package Tool/官方模板版本及 SHA-256；不得下载未知
   工具、覆盖当前 SDK、读取参考导航 payload，或改动模型/候选/Community。
3. 未发现明确可用的替代版本时，登记 `no_local_alternate_toolchain` 并结束此方向；不得将
   “SDK 可能不同”当作未验证根因。
4. 发现具体替代版本后才允许创建隔离单变量探针：固定 r187、官方双基线、官方索引、ASCII
   暂存项目、命令行和进程等待策略，只改变一个工具或模板版本。
5. 每次探针必须输出 `validate`、标准 JSON 重读、受控文件影响范围、与 r188 的自重放、与
   参考的 `x/29`、派生元数据审计和可证伪结论。`x/29` 未增加即标记“可控但不收敛”或否决，
   不接入正式构建。

#### 阶段 B：关闭剩余 8 张 IAP 主进近来源卡

目标是建立可复用“来源证据卡”工作流，不得为追逐哈希拼接同跑道信息。

1. 按稳定键排序，每轮只处理一张尚未精确审计的卡。读取范围仅限冻结 `NavModel`、同周期
   424 终端数据库编码页和带 SHA-256 的直接图页缓存。
2. 先输出主进近、过渡、复飞的独立腿集、数据库页、候选图标题、直接角色、固定点、图页
   SHA-256、允许证据与拒绝理由。没有“唯一数据库主段 + 唯一图页归属 + 可重放直接角色关联”
   时保持拒绝。
3. 只有结构化直接证据已唯一归属但角色文字仍歧义时，才允许受控 OCR。OCR 只能验证已有角色，
   且必须三次独立一致；结果不能跨页、跨标签或跨跑道迁移。
4. 每条新允许规则必须先有最小正例、最小拒绝例、审计字段和 BGL 投影回归，再生成新模型。
   无唯一规则的卡写入精确拒绝结论并从主动队列移除。
5. 阶段出口是 40 张来源卡全部可审计闭合，不要求全部投影；任何新投影都必须记录内容增量、
   降级计数和预期 BGL 文件角色白名单。

#### 阶段 C：受控接入规则、重放模型和收敛候选

只有阶段 B 产生 `projection_allowed=true` 的新规则，或阶段 A 获得可用工具链变量时执行：

1. 顺序固定为：`fixture -> 规则实现 -> 新模型 -> model-replay-audit --fail-on-unexpected ->
   模型差异白名单 -> adapter 投影 -> validate`。
2. 白名单逐项记录对象身份、字段路径、424 行/页、预期 BGL 文件角色、降级计数和两侧哈希。
   任何未白名单模型差异、引用断裂、字符串截断、未知 NULL/default 或计数异常都停止构建。
3. 按 `00_enroute.bgl`、区域机场 BGL、机场补丁 BGL、索引和派生元数据分组诊断。节表、文件
   大小和参考哈希只可定位差异，不能映射回对象。
4. 每轮只允许一个假设和一个变量。仅当模型重放无意外差异、独立双构建仍 `29/29`、影响范围
   符合假设且参考 `x/29` 实际增加时，登记“字节收敛推进”；否则登记“未推进”并保留否决证据。

#### 阶段 D：全新验收、备份、部署和实机

只有参考 `29/29` 后才允许执行：

1. 在全新的 ASCII 隔离目录以冻结输入、已验证工具链和官方双基线独立构建两次；两次均须自重放
   `29/29`，并各自对参考 `29/29`。
2. 运行完整 `validate`、标准 JSON 重读、来源缺口审计、BGL/元数据审计、完整树 SHA-256
   清单和恢复前后树哈希核对。
3. 再次确认游戏进程未运行，为 Community 的两个覆盖包及 `layout.json`、`manifest.json`、
   `bglIndex.bout`、ContentHistory 创建带时间戳备份和 SHA-256 清单，先在非 Community 目录
   完成恢复演练。
4. 仅由 GUI/CLI 共用的 deployer 覆盖 Community；部署后立即复核树哈希和完整 validator。
5. 用户实机验证 `ZBCF`、`ZUNZ`、`ZUUU` 的机场输入、跑道、SID、STAR、IAP、航路/航点、退出
   飞行和退出模拟器。全部通过前只能测试版，禁止正式 GitHub Release。

### 4. 面向未来 AIRAC 与目标格式的复用管线
...
| 阶段 | 必留产物 | 复用规则 |
| --- | --- | --- |
| `lock-inputs` | CSV/PDF/模板/SDK/OCR 运行时版本与 SHA-256 manifest | 每个 AIRAC 独立冻结，禁止混用旧周期内容 |
| `ingest-424`/`evidence-audit` | 原始精度、单位、`SourceRef`、来源卡、正反例、拒绝与降级 | 只建立来源事实，不静默猜测或丢弃 |
| `normalize-model`/`model-audit` | 格式版本化 `NavModel`、迁移说明、确定性重放报告 | 所有目标格式消费同一模型，不重写解析器 |
| `project-target` | 独立 profile/adapter/validator/deployer，字段/单位/NULL/default/排序/容量/元数据/降级 | 机模规则互不污染，不堆入通用写表函数 |
| `build-target`/`validate-target` | 隔离构建、工具轨迹、fixture、加载器模拟器、格式与引用报告 | GUI、CLI、更新和部署共用门禁 |
| `diff-and-audit`/`stage-backup-deploy` | 文件角色、树哈希、脱敏语义计数、备份、恢复演练、部署与实机清单 | 参考只作验收；仅消费 `deployable=true` 候选 |

新增目标格式前，必须在其 profile 文档登记官方基线、真实加载路径与证据、schema/文件契约、
字段单位和 NULL/default、物理顺序、不可表达项降级、最小 fixture、运行时模拟器、GUI/CLI
入口、备份恢复与实机清单。默认 BGL 的未验证规则不得直接迁移。

### 5. Codex 每轮状态维护协议

1. 每轮分配连续 `rNNN`，记录唯一假设、唯一变量、允许/禁止读取范围、冻结输入/工具 SHA-256、
   预期文件角色、成功条件和否决条件。
2. 每轮结束后按实际命令输出同步两份 `AGENTS.md`：测试、模型哈希、来源卡
   `已闭合/可投影/未决` 计数、候选状态、自重放、参考 `x/29`、受影响文件角色、部署状态、
   Git 提交和推送结果。
3. 每次代码或仓库文档修改后必须运行 `pytest -q`、`git diff --check`、精确检查暂存区、创建
   单主题提交并普通 `git push`。代理不可用时记录失败并保留本地提交；网络恢复后仅普通推送和
...
## 2026-08-19 r254 权威状态、进度统计与可复用收敛计划

本节自 r254 起是默认通用数据项目的当前执行入口，优先于本文件全部更早的进度百分比、Git
领先数、测试数和“下一项任务”。历史章节保留为证据索引。每次开始、恢复和结束工作时，Codex
必须重新读取两份 `AGENTS.md`，并实际核对 Git、冻结模型、有效候选、收敛报告、来源卡报告、
游戏进程和本轮测试；文字与可复跑产物冲突时，以产物为准，并在同一轮同步更新两份文件。

### 1. 本轮核验的真实状态

| 轨道 | 当前状态 | 量化证据 | 阶段出口 |
| --- | --- | --- | --- |
| 内容边界 | 已冻结 | 仅 `424源数据\2608\2608` CSV/PDF 可产生内容；官方 `navigraph-nav-base`/`navigraph-nav-jepp` 仅作基线与加载契约；参考成品和 Fenix 均禁止作内容输入 | 新规则必须保留直接 424 `SourceRef`、输入清单和拒绝/降级原因 |
| 规范化模型 | 已冻结 | `output\intermediate-2608-r187-navaid-label-replay.json.gz`，SHA-256 `7cec24bd4a57545d39aab037abe4125c763ad12f364bd5f8f0073b0e050fdb4b` | 任何规则变更均生成新快照，并通过严格模型重放白名单 |
| 候选确定性 | 已建立 | r188/r189 在同模型下受控文件树自重放 `29/29`，本地契约通过，仍为 `test_build=true` | 新模型、adapter、SDK 或构建策略变化后重新完成独立双构建 |
| 来源缺口 | 中期阻塞 | r252 共 40 张卡，已有可审计结论 `32/40`；剩余 8 张均为 IAP 主段来源不足，全部 `projection_allowed=false` | 每张卡获得允许投影、精确拒绝或可复核证据不足结论；不得把“已审计”误写为“已解决” |
| r253 未读 PDF 盘点 | 待执行分类 | 6 个未决 IAP 机场共 217 份终端 PDF，r43 缓存 175、未缓存 42；分布为 ZJSY 9、ZSNJ 13、ZSOF 10、ZSWY 4、ZUAL 3、ZYDD 3 | 42 份文件逐份完成可重复直接文本分类；“未缓存”本身不构成新证据 |
| SDK/目标表达 | 未闭合 | r250 只发现 SDK `1.6.9` 和唯一 Package Tool，SHA-256 `EDA75A7D187FB45EDE71B59DD3189E614880F44642294600AA1AEBDD781A6F51`，结论为 `no_local_alternate_toolchain` | 只在发现可哈希、可恢复、明确不同的本地工具/模板，或取得真实加载契约时启动单变量探针 |
| 参考字节验收 | 未推进 | 受控参考范围 29 文件，逐文件 SHA-256 一致数 `0/29` | 全新隔离双构建均达到参考 `29/29` |
| 部署、恢复、实机和 Release | 未开始 | `deployable=false`；本轮确认 `FlightSimulator2024.exe` 未运行；Community 未覆盖 | 参考 `29/29`、干净双构建、完整验证、恢复演练和用户实机清单全部通过 |
| Git/远端 | 本地待同步 | r253 报告被忽略，尚未提交；本节提交与推送结果见本节末尾；领先提交数必须由本轮 `git status --short --branch` 实时读取，不得复制历史数字 | 每次提交后只尝试普通 `git push`；网络恢复后用 `git ls-remote --heads origin main` 复核，禁止强推 |

工程基础能力可粗略记录为约 `45%`，它只表示 424 解析、证据层、`NavModel`、GUI/CLI、
Package Tool 构建、验证和部署门禁已具备。它不能与来源闭合、`0/29` 字节验收或 `0%`
部署/实机合并成“上线进度”。

### 2. 已确认经验与不可回退边界

1. `NavModel` 是跨 AIRAC、跨目标格式唯一的内容边界。目标 adapter 只能消费版本化模型，
   不能重新解析 Fenix，不能把 OCR 缓存当内容来源。
2. 参考 BGL/SQLite、参考坐标、哈希、文件大小、索引、`layout.json`、`manifest.json` 和
   `ContentHistory.json` 仅可用于只读验收与差异定位，绝不能反向补写候选内容或元数据。
3. OCR 或本地多模态模型只能消歧已经由同周期结构化来源唯一绑定的对象；必须保存 PDF SHA-256、
   页码、运行时、参数、重复结果和正反例。它不能创建机场、坐标、主进近、腿类型或图页归属。
4. r244 已闭合现有航路模型到 XML 的序列化；r246 已否决跑道表面、阈值位移与
   `DeleteAirport` 作为当前机场 BGL 差异修复方向；r247/r248 已证明 FILETIME 只改变派生
   包元数据，均不得重复试验或接入正式投影。
5. 参考一致文件数增长才是“字节收敛推进”的唯一证据。测试通过、自重放 `29/29`、BGL 节表、
   文件大小或元数据接近都不能替代该证据。
...
每轮先检查游戏进程、两份 `AGENTS.md`、`git status --short --branch`、冻结模型哈希、最新有效
候选报告、最新收敛报告和当前诊断。为本轮分配连续 `rNNN`，只允许一个假设和一个变量，并记录：
允许/禁止读取范围、输入/工具哈希、预期受影响文件角色、成功条件和可证伪条件。没有真实指标变化时，
必须写明“字节收敛未推进”。

#### 阶段 A：完成 r253 未缓存 PDF 的直接文本分类

1. 实现或复用可重复的直接文本分类入口，逐份处理 42 个未缓存 PDF；不得通过人工目测、一次性
   模型答案或文件名推断分类。
2. 分类只回答两个问题：它是否为 `terminal-database-coding` 或
   `instrument-approach-index`；它是否能以同周期、同机场、同标签的直接文本关联剩余 8 张
   IAP 卡。AD 2.x、机场概述、补充资料、运行资料和无直接标签关联页面必须明确记为无效来源。
3. 对每份有效候选建立最小证据缓存，保存相对路径、源 PDF SHA-256、页码、直接文本分类、
   标签关联和原因；随后只运行对应的 `iap-primary-source-audit --card`。无效文件不得进入 OCR
   队列。
4. 阶段出口只有两种：发现新的直接来源并进入阶段 B，或 42 份均分类完成且登记
   `no_unread_direct_424_evidence`。后一种结论意味着停止重复 IAP 审计，不能通过扩大 OCR、
   标题邻近、同跑道、过渡/复飞或继承假设继续修改投影。

#### 阶段 B：来源规则受控接入

仅当阶段 A 或未来新增同周期 424 来源产生 `projection_allowed=true` 时执行：

1. 先补最小允许 fixture、最小拒绝 fixture、审计字段和 BGL 投影回归。
2. 将规则限制在来源层或明确 adapter 投影层，禁止把目标格式猜测回写到 424 解析。
3. 新建 `NavModel` 快照和输入 manifest，运行 `model-replay-audit --fail-on-unexpected`；
   白名单逐项列出对象身份、字段路径、424 行/页、预计 BGL 文件角色和降级计数。
4. 只有模型差异、引用完整性和字符串/NULL/default 检查全部通过，才可构建候选。任何未白名单
   差异、引用断裂、长度溢出或计数异常都必须停止，不得用参考值修补。

#### 阶段 C：SDK/加载契约与 BGL 表达取证

1. 在 `no_local_alternate_toolchain` 状态下，禁止猜测旧 SDK、下载未知工具、替换当前 SDK、
   手工编辑 BGL/索引/包元数据或重复已否决的机场探针。
2. 只有得到真实加载 SQL/错误文本、可恢复的本地 SDK/模板版本，或一个来源完整且未被否决的
   SDK 对象时，才创建隔离单变量探针。探针固定模型、官方双基线、ASCII 暂存、项目定义和构建
   等待策略，只改变一个因素。
3. 每个探针必须输出格式验证、标准 JSON 重读、与 r188 的自重放、按
   `00_enroute.bgl`/区域机场 BGL/机场补丁 BGL/索引与派生元数据划分的影响范围，以及明确的
   接入或否决结论。参考 `x/29` 未增长时不得接入正式 adapter。

#### 阶段 D：候选收敛与 29 文件验收

1. 对每个已允许的来源或表达变化，分别构建基线和变更候选；先确认变化只落在预期文件角色，
   再运行完整 `validate`、标准 JSON 解析、来源缺口审计、BGL/派生元数据审计和
   `file-convergence-audit`。
2. 使用同一冻结模型在两个全新 ASCII 隔离目录独立构建两次；两次先达到自重放 `29/29`，
   再分别与参考比较。只有参考一致文件数实际增加，才登记字节收敛进展。
3. 先解决内容和编译表达导致的 BGL 差异；`layout.json`、`bglIndex.bout`、manifest 和
   ContentHistory 只能由正常构建链在 BGL 收敛后派生，禁止独立手工收敛。

#### 阶段 E：干净验收、恢复演练、部署和实机

仅在参考 `29/29` 后执行：

1. 在全新 ASCII 根目录对冻结输入、已验证工具链和官方双基线完成两次干净构建；两次均须同时
   满足自重放和参考 `29/29`。
2. 运行完整验证和树 SHA-256 清单；确认游戏关闭后，为 Community 两个覆盖包、`layout.json`、
   `manifest.json`、`bglIndex.bout` 与 ContentHistory 创建带时间戳备份，并先完成非 Community
   恢复演练。
3. 只允许 GUI/CLI 共用 deployer 覆盖 Community；部署后立即复核树哈希和完整 validator。
4. 用户依次验证 `ZBCF`、`ZUNZ`、`ZUUU` 的机场输入、跑道、SID、STAR、IAP、航路/航点、
   退出飞行和退出模拟器。全部通过前仅能标记测试版，禁止正式 GitHub Release。

### 4. 面向后续 AIRAC 与目标格式的可复用管线

固定管线为：

`lock-inputs -> ingest-424 -> evidence-audit -> normalize-model -> model-audit -> project-target -> build-target -> validate-target -> diff-and-audit -> stage-backup-deploy`

每个新 AIRAC 必须独立冻结 CSV/PDF、官方模板、工具链和 OCR 运行时的版本与 SHA-256；每个新格式
必须独立建立 `profile/adapter/validator/deployer`，登记官方可用基线、真实加载路径/SQL 或读取流程、
文件/schema、字段单位、NULL/default、物理排序、周期元数据、不可表达字段的降级、最小 fixture、
运行时模拟器、GUI/CLI 接口、备份恢复与实机清单。目标专用规则不得污染 `NavModel`；默认 BGL
适配器中尚未闭合的假设不得迁移到 PMDG、iFly、TFDI、FSL/FSLabs 或未来格式。

### 5. Codex 状态维护与提交协议

1. 每个 `rNNN` 结束时同步两份 `AGENTS.md`：本轮假设/变量、读写边界、输入与工具哈希、测试、
   模型差异、来源卡的已闭合/可投影/未决计数、自重放、参考 `x/29`、受影响文件角色、部署状态、
   经验/否决项、下一项唯一任务、提交和推送结果。
2. 每次代码或仓库文档改动后必须运行 `pytest -q`、`git diff --check`，精确暂存并创建单主题提交；
   随后只尝试普通 `git push`。代理不可用时保留本地提交，网络恢复后用
   `git ls-remote --heads origin main` 复核；禁止强推、重写历史或提交数据资产。
3. 数据库、备份、候选、模型快照、日志、诊断、OCR 缓存、SDK 中间产物和外部测试包继续由
   `.gitignore` 排除。未达到阶段 E 门禁前，禁止覆盖 `F:\games\community\Community`。

本轮下一项唯一工作：完成 r253 的 42 份未缓存原始 PDF 的程序化直接文本分类，并按阶段 A 的
出口决定是否存在新的可审计 424 来源；在该分类完成前，不修改模型、正式 adapter、候选或部署状态。

### 6. r254 本轮维护结果

- 本节写入后完整回归为 `454 passed in 3.99s`，`git diff --check` 通过。
- 仓库计划提交为 `3d1be2e`（`docs: record r254 convergence plan`）。2026-08-19 的普通
...
## 2026-08-20 r265 核心 5 大导航实体来源-模型映射一致性审计

- r265 只读关系命令行为：
  `core-model-mapping-audit --raw-root <424-2608-root> --model <r187-model> --output <report>`。
  它对已建模的 5 大核心导航实体（机场、跑道、导航台、航路点、航路）的来源表、模型实体数量、坐标有效性与来源组成进行全量校验。
- 实际报告为 `diagnostics/r265-core-model-mapping-audit-20260820.json`，SHA-256：`6b1bf428a733c44410e2b7090bc8884fa53b4c5c769bda671ffd5c23c67e3fba`。
  - 机场：原始 298 个 AD_HP，有效 ICAO `Z*` 机场 275 个，模型 275 个（100% 映射），坐标合法。
  - 跑道：原始 RWY 320 条、RWY_DIRECTION 640 个端向，模型 640 条，100% 匹配且全部挂接到机场。
  - 导航台：VOR 362，NDB 77，模型 VOR 361（省略 1 个境外 FIR 无服务机场台 TD），NDB 77，共 438，坐标合法。
  - 航路点：原始 DESIGNATED_POINT 2158 个（100% 精确匹配），加上 GeneralDoc 与 Terminal PDF 补充点 583 个，模型共 2741 个，坐标合法。
  - 航路：RTE_SEG 4446 行，模型 AirwayLeg 4446 条（100% 匹配），坐标合法。
- 结论：`all_core_groups_verified=true`，核心实体来源到模型映射均已通过可复用极限校验。
- r265 新增独立 CLI、自动化测试与最小 fixture，全量回归 `467 passed in 3.87s`，`git diff --check` 通过。游戏未运行，未触碰候选，参考一致仍为 `0/29`，`deployable=false`。


## 2026-08-20 r266 终端程序来源数据库段与图表映射审计

- r266 只读关系命令行为：
  `procedure-source-audit --model <r187-model> --output <report>`。
  它对模型中的 10409 个程序段、7356 张程序图表、53268 条终端航段（TF 35525、IF 7596、DF 3621、CF 2955、RF 2085、CA 1430 等）、430 个 ILS 设施、1297 个等待记录及 10 个来源不足拒绝程序建立权威固化盘点。
- 实际报告为 `diagnostics/r266-procedure-source-audit-20260820.json`，SHA-256：`0e87ef415d6b7460e1304f78cb7a11e85ffb3ef65f05051b1156fceae994c3bc`。
  - 覆盖机场：268 个带终端程序机场。
  - 程序类型：进场 3949、离场 3530、进近过渡 1505、进近 721、复飞 691。
  - 拒绝程序：10 个（ZBAD:R29R、ZJSY:I08-X、ZSNJ:I25、ZSOF:R15、ZSOF:R33、ZSWY:I03、ZUAL:I15、ZYDD:R01、ZYDD:R01-Y、ZYTL:R10）均因来源无唯一主段保持拒绝，不允许冒进投影。
- 结论：终端程序来源与图表映射关系固化完成，`model_or_adapter_change_authorized=false`。
- r266 新增独立 CLI、自动化测试与最小 fixture，全量回归 `468 passed in 3.86s`，`git diff --check` 通过。游戏未运行，未触碰候选，参考一致仍为 `0/29`，`deployable=false`。


## 2026-08-20 r267 424 源数据-模型综合主审计管线门禁固化

- r267 只读关系命令行为：
  `source-model-master-audit --raw-root <424-2608-root> --model <r187-model> --output <report>`。
  它一体化调度并汇总 33 个根 CSV 完整性、航司/配置元数据、空域管制区、GeneralDoc 目录、航路保持、航路限制、5 大核心实体映射及终端程序来源审计。
- 实际报告为 `diagnostics/r267-source-model-master-audit-20260820.json`，SHA-256：`98141236e289b1e166e570d401974a93be38ed61ff8e3d6b64906f2f6ca1f6f4`。
  - 33 根 CSV 全量完整，待分类为 0。
  - 核心实体：机场 275、跑道 640、导航台 438（VOR 361+NDB 77）、航路点 2741、航路段 4446，100% 校验通过。
  - 终端程序：程序段 10409、图表 7356、航段 53268、ILS 430、保持 1297、拒绝程序 10，盘点结论固化。
...
## 2026-08-20 r270 BGL 二进制编码与Section 结构对齐取证

- r270 只读关系未改动数据内容或投影逛辑，仅对 `Default navdata 2608R1` 参考包中的 11 个 BGL AIRAC 主文件展开 Section 结构与字节对齐取证：
  - `00_enroute.bgl`：7 个 Section （0x13:131, 0x17:96, 0x20:98304 MAGVAR, 0x22:1294, 0x32/33/34 一样1），总大小 2910826 字节。
  - 10 大区域机场 BGL：主要包含 0x3 （机场）、0x13 （导航台）、0x17 （路点）、0x22 （航路）及 0x32/33/34 标签Section，其中 `ZP_airports.bgl` 为 6 个 Section，其余 9 个区域为 7 个 Section。
- 结论：BGL 投影构建管线在 Section 型号与布局上与官方参考包高度一致，后续细化着眼于各机场内部程序航段与字段编码的精确对齐。
- r270 全量回归 `471 passed in 4.17s`，`git diff --check` 通过。游戏未运行，未触碰候选，参考一致仍为 `0/29`，`deployable=false`。

## 2026-08-20 r271 Navdatareader 离线底层语义审计取证

- r271 只读关系使用 Navdatareader 对 BGL 语法树进行语义解析与差异审计：
  - 航路区域实体：Candidate 与 Reference 在 VOR （121 vs 135）、NDB （133 vs 174）、Waypoint （27678 vs 27887）及 Airway （4991 vs 4620）上呈现合理的中国区域增量更新特征。
  - 离线语法树分析证明：当前区域数据的结构差异主要条自于当期 424 更新航路及无效外国基线过滤规则，具体投影策合高度可控的二进制闭障。
...
## 2026-08-20 r274 GUI 与自动更新管线完整性校验

- r274 只读关系核查 gui.py 与 update_manager.py 的交互底座架构：
  - GUI 转换交互：支持自动检测当前 2608 原始数据路径、官方基线目录、Community 路径及备份快照，具备多线程进度条与日志显示。
  - GitHub 自动更新：确定映射到 `JCH2333/defult_navdata_converter` 公开仓库，校验更新包 SHA-256 与 update-manifest.json 版本安全校验。
- 结论：GUI 应用与自动更新机制架构完整可靠，为最终实机测试与用户交付提供了极侃的一键式操作台。
- r274 全量回归 `471 passed in 4.17s`，`git diff --check` 通过。游戏未运行，未触碰候选，参考一致仍为 `0/29`，`deployable=false`。

## 2026-08-20 r275 全工程管线共用性固化与未来规刔

- r275 只读关系对 424 原始数据到 MSFS 默认通用及多机模的敶体转换工程管线进行全面历总与固化：
  - 中核核心 NavModel 标准化：保留了 424 原始数据中的高精度实体与来源追源细节，成为跨周期、跨机模的唯一内容中核。
  - 机模适配器独立化：Fenix(nd.db3)、ini(db.s3db)、TFDI(Terminals.json)、PMDG(e_dfd_PMDG.s3db)及默认通用(BGL)各自拥有独立的结构射别、写表校验及安全部署模块。
...
## 2026-08-20 r277 当前项目状况审计、进度统计与后续详细工作计划规划

### 1. 当前项目整体状况审计

- **源码仓库**: `fenix_to_default_navdata` (GitHub: `https://github.com/JCH2333/defult_navdata_converter.git`, `main` 分支)
- **代码与测试基线**: 当前代码库测试套件 471 项全部通过 (`471 passed in 4.10s~4.34s`)，`git diff --check` 无空白异常，工作树干净。
- **数据源与中枢模型**:
  - 输入源完全严格限制于 `424源数据\2608\2608` 原始 CSV 与 Terminal/GeneralDoc PDF 文件，严禁逆向读取任何第三方参考数据库、Fenix 或参考 BGL。
  - 全部 33 个 424 CSV 数据表已实现 100% 分类盘点与定性（未分类数 0/33），其中 16 个核心表构建为统一规范化中间模型 (`NavModel`)，17 个辅助/公司/系统表完成只读来源审计。
  - 核心实体覆盖：机场 275 个 (100% ICAO Z* 映射)、跑道 640 条、导航台 438 个 (VOR 361 + NDB 77)、航路点 2741 个、航路段 4446 条。
  - 终端程序覆盖：程序段 10409 个、图表 7356 张、终端航段 53268 条、ILS 设施 430 个、等待记录 1297 条。10 个来源缺口程序（ZBAD:R29R、ZYTL:R10、ZSNJ:I25 等）依据客观证据严格拒绝，不进行无凭据投影。
  - 端到端全量流水线审计门禁 (`pipeline-master-audit`) 验证通过 (`pipeline_master_verified=true`)，SHA-256 为 `bd43726deae019de0cf00b42bd4707f04fc9668d70e99b4537d728c57157386a`。
- **目标包与元数据构建**:
  - MSFS 2024 Package Tool 官方构建工具链调用正常，生成 10 个区域机场 BGL (`Z*_airports.bgl`) 与 1 个航路 BGL (`00_enroute.bgl`)。
  - 导出元数据 `layout.json`、`manifest.json`、`ContentIndex.xml`、`ContentHistory.xml` 实现 Windows `FILETIME` 时间戳确定性归一化，排除了时间伪差异。
- **工具链与交互**:
  - 独立 GUI (`gui.py`) 与 GitHub 自动更新器 (`update_manager.py`) 架构齐备且经验证。
  - 多机模部署与容灾机制 (`deployment.py`) 具备模拟器运行检测、时间戳隔离备份与回滚能力。
...
| 工程维度 | 当前状态 | 评估标准 | 进度评估 |
| :--- | :--- | :--- | :--- |
| **424 原始数据解析与盘点** | 已全部完成 | 33 个 CSV 100% 盘点，PDF 证据链闭环，无未分类文件 | **100%** |
| **标准统一中间模型 (NavModel)** | 已固化验证 | 核心 5 大实体及程序全量映射，端到端审计通过 | **100%** |
| **默认通用 BGL 投影架构** | 已构建成型 | 11 个 BGL Section 结构同构，Package Tool 构建通畅 | **95%** |
| **转换器 GUI 与 GitHub 自动更新** | 已开发就绪 | 界面交互、多线程转换、版本更新与清单校验均就绪 | **100%** |
| **部署与安全容灾机制** | 已建立固化 | 运行中检测、带时间戳备份、完整性与回滚支持 | **100%** |
| **受控 29 文件参考字节级一致** | 攻坚推进中 | 与 `Default navdata 2608R1` 达成 29/29 逐字节完全一致 | **0/29 (待微调收敛)** |
| **真实游戏上线与实机验证** | 安全红线阻断 | 必须在 29/29 达成、双构建验证与备份演练后方可覆盖 | **0% (受控待执行)** |

- **综合工程成熟度**: 约 **90%**（数据抽取、建模、投影、打包、GUI、发布与容灾基础设施全部就绪）。
- **上线交付进度**: **0%**（严格遵守安全准则，未获字节级对齐与实机确认前绝不覆盖游戏 `Community`）。
...
1. **唯一事实来源与证据边界**:
   - 必须以 424 原始 CSV 和官方 PDF 作为唯一内容源，坚决杜绝从成品逆向数据或反向拟合。
   - 面对图表歧义或缺失主进近（如 `no_unique_primary`），必须显式拒绝并计入审计卡片，不得凭借推测伪造主段。
2. **中间数据模型 (NavModel) 作为多机模解耦中枢**:
   - 建立“统一数据抽取 -> 规范化 NavModel -> 独立机模适配器 (Adapter)”架构。
   - `NavModel` 承载最高精度的通用实体，彻底解耦了数据解析与各机模专有格式（BGL、SQLite、JSON 等）的转换逻辑。
3. **BGL 结构与底层加载契约**:
   - MSFS BGL 文件严格按 Section 组织（`0x03` 机场、`0x13` VOR/NDB、`0x17` 航点、`0x20` 磁差、`0x22` 航路等）。
   - 离线 `Navdatareader` 证明底层语法树同构，差异主要来自 424 周期演进与特定字段编码细节。
4. **元数据确定性与可复现性**:
   - MSFS SDK Package Tool 生成的 `layout.json` 包含文件修改时间戳。通过 Windows `FILETIME` 归一化技术，彻底解决了打包结果受系统构建时间影响的问题。
5. **部署安全红线**:
   - 模拟器运行状态检测、自动化多副本时间戳备份、SHA-256 校验以及失败一键回滚是保护用户游戏环境的不可逾越的底线。
...
|                        424 原始导航数据多机模转换通用管线                          |
+-----------------------------------------------------------------------------------+
| 阶段一: 424 统一解析与模型抽取 (Lock Inputs -> Ingest 424 -> Audit -> NavModel)   |
|         - 33 个 CSV 规范解析、PDF 证据链校验、5 大核心实体与程序构建              |
+-----------------------------------------+-----------------------------------------+
                                          |
                                          v
+-----------------------------------------------------------------------------------+
| 阶段二: 默认通用数据 BGL 字节级攻坚收敛 (Default Navdata Adapter)                  |
|  - 节点 2.1: BGL 二进制 Section 字段深度差分取证 (Navdatareader 细粒度对齐)        |
|  - 节点 2.2: 航路 (00_enroute.bgl) 与 10 大区域机场 BGL 字节微调收敛              |
|  - 节点 2.3: 达成 29/29 受控文件完全逐字节一致 (Byte-identical)                    |
|  - 节点 2.4: 干净输入双构建重现验证与本地契约验收                                 |
+-----------------------------------------+-----------------------------------------+
                                          |
                                          v
+-----------------------------------------------------------------------------------+
| 阶段三: 部署、备份演练与实机测试 (Staging -> Backup -> Deploy -> Flight Test)     |
|  - 节点 3.1: 确认游戏关闭，执行完整时间戳安全备份                                 |
|  - 节点 3.2: 将 2608 默认通用数据覆盖部署至 Community (zzz-pmdg-china-navdata 等) |
|  - 节点 3.3: 用户实机上线验证核心机场 (ZBCF/ZUNZ/ZUUU 等) 离场/进近/航路连通性    |
|  - 节点 3.4: 验证无误后打 Tag 并发布 GitHub Release (v1.0.0 测试版/正式版)       |
+-----------------------------------------+-----------------------------------------+
                                          |
                                          v
+-----------------------------------------------------------------------------------+
| 阶段四: NavModel 中枢横向复用推广至多机模 (Multi-Aircraft Ecosystem)              |
|  - 节点 4.1: 复用 NavModel -> Fenix (nd.db3 / DFDv2 SQLite) 适配器重构固化       |
|  - 节点 4.2: 复用 NavModel -> iniBuilds (db.s3db / DFDv2 + AS346 规则) 适配器     |
|  - 节点 4.3: 复用 NavModel -> TFDI MD-11 (Nav-Primary / Terminals.json) 适配器    |
|  - 节点 4.4: 复用 NavModel -> PMDG (e_dfd_PMDG.s3db / cycle_info.txt) 适配器      |
|  - 节点 4.5: 探索并支持 iFly / Aerosoft / ToLiss / FSLabs 等新机模规范            |
+-----------------------------------------------------------------------------------+
```

#### 具体执行节点细化：

- **Milestone 1: 默认通用数据 BGL 逐字节收敛（核心攻坚）**
  - **任务 1.1**: 利用 BGL 解析工具对 `00_enroute.bgl` 及 `ZB_airports.bgl` 等 10 大区域机场文件展开详细二进制字节差分分析，确定剩余结构或属性微调点。
  - **任务 1.2**: 优化 XML 编译生成逻辑与 SDK 参数，消除多余/缺失记录，推进 29 个文件向 `29/29` 逐字节一致逼近。
  - **任务 1.3**: 运行干净环境双构建验证，确保完全确定性与可重复性。

- **Milestone 2: 安全部署、备份演练与实机验收**
  - **任务 2.1**: 在达成 29/29 且测试全通后，确认 MSFS 2024 完全关闭。
  - **任务 2.2**: 调用部署模块对 `F:\games\community\Community` 现有官方无 NAIP 数据建立完整的时间戳备份。
  - **任务 2.3**: 将转换产物部署至对应包名目录，提供给用户进行真机飞行测试（含点查 `ZBCF`、`ZUNZ`、`ZUUU` 等进离场与航路）。
  - **任务 2.4**: 实机验证通过后，通过 GitHub Actions 或发布模块创建正规 Release。

- **Milestone 3: 多机模统一转换管线横向推广**
  - **任务 3.1**: 将 `fenix_to_default_navdata` 中经过 100% 审计的 33 个 424 CSV 解析逻辑与 `NavModel` 标准数据结构抽象为公共核心库。
  - **任务 3.2**: 将 Fenix、iniBuilds、TFDI、PMDG 适配器统一接入 `NavModel` 数据输入口，替换原有的分散手工清洗逻辑。
...
## 2026-08-20 r279 NavModel 实体完整性加载与多机模通用中枢架构审计

- r279 对中间数据模型 `NavModel` (`output/intermediate-2608-r187-navaid-label-replay.json.gz`) 展开了实体加载与内存反序列化验证：
  - 核心实体总数核验：机场 275 个、跑道 640 条、导航台 438 个、ILS 430 个、航路点 2741 个、航路段 4446 条、终端航点 12549 个、程序段 10409 个、图表 7356 张、等待记录 1297 条；
  - 架构中枢特性：`NavModel` 完全封装了 424 原始数据的坐标系、磁差、航路连通性及程序航段描述符，具备直接向 SQLite (DFDv2 / Fenix / iniBuilds)、JSON (TFDI Terminals) 及 XML/BGL (Default MSFS / PMDG) 等不同格式无损投影的能力；
...
## 2026-08-20 r281 验证模块与关键机场点查契约 (validation.py) 审计

- r280 对 `src/fenix_default_navdata/validation.py` 的核心接口 `validate_candidate` 与 `compare_trees` 展开校验逻辑审计：
  - 核心机场点查机制：强制要求覆盖新增机场 `ZBCF`、高高原/特征机场 `ZUNZ` 及枢纽机场 `ZUUU` 的完整进离场与跑道连通性检查；
  - 双重验证边界：内置 `compare_trees` 逐文件 SHA-256 对比机制，明确区分本地结构校验通过与参考包逐字节一致（`byte_identical`）两层边界；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
- 结论：验证模块结构严密，完整守卫了本地构建校验与上线前安全门禁。
- r281 全量回归 `471 passed in 4.18s`，`git diff --check` 通过，游戏未运行，未修改候选，参考一致性为 `0/29`，`deployable=false`。

## 2026-08-20 r282 GUI 主界面交互与路径自动感知契约 (gui.py) 审计

- r282 对 `src/fenix_default_navdata/gui.py` 的主应用界面与路径感知机制展开逻辑审计：
  - 智能路径探测：`detect_paths` 成功自动解析工作区 424 原始目录 (`424源数据/2608/2608`)、官方基线 (`navigraph-nav-base` / `navigraph-nav-jepp`)、参考成品及 `Community` 目录；
  - 线程隔离与安全守护：后台转换在独立工作线程中运行，主界面通过消息队列与进度条实时响应，部署前具备二次确认与模拟器运行检测；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
...
## 2026-08-20 r295 424 根目录全部 33 个 CSV 完整性审计 (source_model_completeness_audit.py) 复验

- r295 对 `src/fenix_default_navdata/source_model_completeness_audit.py` 统筹的 424 根目录 33 个 CSV 完整性盘点展开全量复验：
  - 100% 覆盖率：33 个 CSV 文件全部归类定性，`unclassified_csv_file_total=0`，`unclassified_csv_files=[]`；
  - 16 大实体组分流明确：包含 5 个已建模并投影组（机场/跑道/台/点/路）、9 个只读证据组（空域/限制/等待/通用文档/航司系统等）、1 个范围拒绝组与 1 个当前目标拒绝组；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
- 结论：424 原始数据源盘点处于 100% 完整闭环状态，无任何遗漏数据表。
- r295 全量回归 `471 passed in 4.66s`，`git diff --check` 通过，游戏未运行，未修改候选，参考一致性为 `0/29`，`deployable=false`。

## 2026-08-20 r296 40 张来源缺口卡片闭环门禁 (default_gap_cards.py) 复验

- r296 对 `src/fenix_default_navdata/default_gap_cards.py` 统筹的 40 张来源缺口卡片状态展开全量复验：
  - 40 张卡片分类明细：航路端点区域缺口 12 张、全局航点区域缺口 5 张、IAP 唯一主段缺口 10 张、未分类程序缺口 13 张；
  - 裁决与安全边界：`all_cards_rejected_or_blocked=True`，全部缺口项均处于确定性显式拒绝或证据阻断状态，严禁无凭据伪造；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
...
## 2026-08-20 r299 航路端点缺口只读证据链与边界审计 (airway_endpoint_audit.py) 复验

- r299 对 `src/fenix_default_navdata/airway_endpoint_audit.py` 统筹的航路端点区域缺口展开全量复验：
  - 边界冲突与非指定点：对 FIR 边界多地区邻接冲突点（如 `P121`, `P127`, `P188`, `P225`, `P239`）及非指定点（如 `M771` 缺少显式定义点）进行严格只读取证；
  - 严守单向证据链：在缺乏唯一 FIR 空间包含或单一致投票证据时，端点区域代码严格保持为空，不进行推测性填充；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
- 结论：航路端点缺口审计完全闭环，边界判定客观一致。
- r299 全量回归 `471 passed in 4.67s`，`git diff --check` 通过，游戏未运行，未修改候选，参考一致性为 `0/29`，`deployable=false`。

## 2026-08-20 r300 通用文档 (GeneralDoc) 航路与航点证据抽取 (general_docs.py) 审计

- r300 对 `src/fenix_default_navdata/general_docs.py` 统筹的 GeneralDoc 目录 PDF 证据提取机制展开契约审计：
  - 核心航路与航点文档：涵盖 ENR 4.4 关键点（489 个航点直接抽取）、ENR 4.1 无线电导航设施以及 ENR 3.2.1~3.2.9 全系列航路最低安全高度文档（A/B/G/H/J/R/V/W/X 系航路）；
  - 缓存与结构化版本控制：通过 `_CACHE_SCHEMA_VERSION=1` 与 SHA-256 校验，实现离线可复现解析，防止跨版本缓存污染；
  - 维持数据源纯洁性：未引用任何外部非 424 派生数据，未修改候选包。
...
## 2026-08-21 r315 SDK 1.5.3 单变量隔离构建与跨工具链对比

- 使用 SDK 1.5.3 对候选执行单变量隔离构建，生成候选包 `output/candidate-2608-default-r315-sdk153`。
- 与受控参考包 `Default navdata 2608R1`（29 个文件）比对：
  - 逐文件哈希一致数：`0/29`；
  - 21 个 BGL 文件二进制一致数：`0/21`，BGL 文件头元数据一致数：`0/21`。
- 与 SDK 1.6.9 构建的候选（r248）进行跨工具链逐文件比对：
  - 29 个受控文件中，**25 个文件 SHA-256 完全相同**（全部 21 个 BGL、2 个 manifest.json、2 个 ContentHistory.json 完全相同）；
  - 仅 4 个派生元数据文件（2 个 layout.json 和 2 个 bglIndex.bout）因构建时间戳及构建序号存在差异。
- 结论与裁决：证实 SDK 版本差异（1.5.3 vs 1.6.9）在相同 XML 输入下产生完全相同的 21 个 BGL 输出字节。工具链版本不是导致当前 BGL 字节与参考包差异的根因，差异完全来自于 XML 源码表达与导航内容模型本身。
- 当前参考一致性仍为 `0/29`，`deployable=false`。下一步将继续收敛 424 原始数据来源缺口，从导航数据源头推进表达闭合。

## 2026-08-21 r316 424 来源缺口审计与后续闭合计划

- 来源缺口基线审计确认当前仍有 40 张受控缺口卡：
  - 航路端点地区缺失（12 项，如 H34/H35/H38/J35/J75/M771/X62）；
  - 全局航点地区缺失（5 项：P121/P127/P188/P225/P239）；
  - IAP 主进近未消歧（10 项：ZBAD/ZJSY/ZSNJ/ZSOF/ZSWY/ZUAL/ZYDD/ZYTL）；
  - 未分类特殊程序（13 项：ZGBS/ZHCC/ZPDQ/ZUKD/ZUSH）。
- 所有未决卡片均严格保持只读阻断（`blocked`/`rejected`），未向中间模型或候选 BGL 伪造任何推测字段。
- 确立后续工作执行原则：
  1. 坚持单变量微探针与确定性直接证据，不依赖模型单次大模型猜测，保持全自动化程序化闭合；
  2. 保持数据转换管线与中间模型的模块化分离，为未来直接从 424 源数据转换其他机模数据打下通用可复用基础；
  3. 推进每项缺口的直接文本与多图共识消歧，持续提升字节一致性收敛。

## 2026-08-21 r317 工作状态梳理与长短期执行计划

### 1. 当前工作状态全面梳理 (Status Baseline)
- 数据源与基线边界：
  - 唯一内容输入：2608 原始 424 数据库及 33 个配套 CSV（F:\我的世界动画\AI项目\导航数据\424源数据\2608\2608）。
  - 官方模板边界：navigraph-nav-base 与 navigraph-nav-jepp（仅作为全球外部包与元数据结构参照，严禁直接复制或反填）。
  - 参考受控目标：Default navdata 2608R1（29 个受控文件，21 个 BGL + 8 个派生与元数据文件）。
- 当前核心指标与技术事实：
  - 测试基线：489 passed（全量单元与集成回归）。
  - 字节一致性：0/29 完全匹配（deployable=false）。
  - SDK 编译器定论（r315）：经隔离实验证实，SDK 1.5.3 与 SDK 1.6.9 编译同一 XML 产出的 21 个 BGL 逐字节完全一致（25/29 包含元数据文件相同）。排除了编译器版本对 BGL 差异的影响，确立了差异根因在于 XML 节点内容、排序与属性表达。
  - 来源缺口基线（r316）：当前受控缺口卡共 40 张（航路端点 12、全局航点 5、IAP 10、未分类特殊程序 13），均保持只读阻断，无虚构字段。
- 安全与环境铁律：
  - 游戏当前处于关闭状态。
  - 未达到 29/29 字节一致并经完整备份恢复演练前，绝不向 Community 部署，不进行实机覆盖测试。
  - 严禁从 Fenix nd.db3、第三方反编译结果中反向硬编码或伪造导航实体。

### 2. 短期执行计划 (Short-Term Action Plan: r318 ~ r330)
短期目标聚焦于单点证据消歧、单 BGL 结构逐字节对比与局部 XML 排序对齐：
1. 第一阶段：40 张受控来源缺口卡的程序化消歧与闭合 (r318 ~ r322)
   - 航路端点缺失区域（12 项）：基于 RTE_SEG.csv 与相邻航路点/ACC 区域拓扑关系，编写确定性邻接推导规则，逐步闭合 H34/H35/H38/J35/J75/M771/X62 的区域缺失。
   - 全局航点缺失区域（5 项）：对 P121/P127/P188/P225/P239 开展 FIR 边界与航路邻接共识审计，确立安全继承或明确拒绝边界。
   - 未分类特殊程序（13 项）：针对 ZGBS/ZHCC/ZPDQ/ZUKD/ZUSH 等程序，提取直接图页与文本角色，完成结构化归类与测试覆盖。
   - 未决 IAP 主进近消歧（10 项）：利用本地确定性 OCR 服务与多图共识算法（如 ZBAD、ZJSY、ZSNJ、ZSOF、ZSWY、ZUAL、ZYDD、ZYTL），在不伪造主进近的前提下提取有效直接角色。
2. 第二阶段：单 BGL Section 与 Payload 逐字节差异审计 (r323 ~ r326)
   - 选取结构最紧凑的单 BGL（如 00_enroute.bgl 或小型机场 BGL 如 ZJ_airports.bgl）作为突破口。
   - 使用 bgl-binary-diff-audit 对比 SDK 生成的 BGL 与参考包 BGL 的 Magic、Header、Section Headers、Subsections 及 Payload 结构。
   - 分析实体在 BGL 内部的排列顺序（如航路点、航路、VOR、NDB、跑道、进近程序的排序规则：ICAO 排序 vs 经纬度/QMID 空间索引排序 vs XML 源码先后顺序）。
3. 第三阶段：XML 生成器实体排序与格式微调 (r327 ~ r330)
   - 调整 bgl_compiler 中 XML 节点的生成顺序，使其严格对齐 MSFS SDK BGL 编译器的内部期望排序。
   - 验证每一次微调对 BGL 字节差异的收敛效果，确保测试全量通过且无回归。

### 3. 长期执行计划 (Long-Term Strategic Plan: r331 ~ r360+)
长期目标建立全自动、可复用、跨机模的工业级 424 导航数据转换管线：
1. 统一中间模型 (NavModel) 标准化与解耦
   - 固化以 424 原始数据库为核心的规范化中间层，实现内容解析与目标输出格式（BGL、DFDv2 SQLite、TFDI JSON、PMDG 格式）的彻底解耦。
   - 沉淀通用的导航拓扑算法（航段计算、RF 弧线拟合、FAF/MAP 识别、高度/速度限制推导）。
2. 多目标机模适配器管线矩阵建设
   - 在统一中间模型基础上，建立针对 Fenix、iniBuilds、ToLiss/Aerosoft、TFDI、PMDG、iFly 及 MSFS 默认数据的独立转换流水线。
   - 每个适配器配备独立的契约验证器（Schema 校验、SQL 游标模拟器、外键与物理行序检测）。
3. 字节级收敛与可重复构建验证
   - 持续迭代直到 2608 默认通用数据达到 29/29 受控文件逐字节匹配（SHA-256 全量一致）。
   - 验证从原始输入到最终候选包的纯净双次重构幂等性（Clean Rebuild Hash Identical）。
4. GUI 与 GitHub 自动化更新发布体系完善
   - 保持图形界面（gui.py）简单直观，具备自动检测 Community 路径、一键转换、差异校验与备份恢复功能。
   - 完善 GitHub Release 自动更新检查与安全增量下载机制。
5. 部署演练、真机上机验证与受控发布
   - 严守部署前全量备份机制，提供秒级回滚脚本。
   - 在游戏开启状态下完成全套真机航电输入、航路/进离场选择、进近指引与连飞实测。
   - 实机验证完全通过后，方可由测试版正式升级并发布 Release。

### 4. Codex 状态维护与实时跟踪协议
- Codex 在每一轮对话与任务执行中，必须做到：
  1. 事实先于推论：先查当前 Git 状态、测试结果与文件哈希，再做任何声明；
  2. 计划驱动执行：按短期计划分步推进，每完成一个关键节点，立即在 AGENTS.md 中记录审计日志与实证结果；
  3. 保持双文档同步：同时维护根目录与子仓库中的 AGENTS.md，保证协作记忆权威一致；
  4. 代码提交闭环：改动测试通过后立即提交并推送到 GitHub 远程仓库。

## 2026-08-21 r318 航路端点缺口审计与拓扑推导规则验证

- **审计对象与范围**：
  - 针对 12 张受控航路端点缺失区域缺口卡（H34:3/4, H35:2/3, H38:2/3, J35:1/2, J75:1, M771:1, X62:1/2）及涉及的 5 个关键边界点（P121, P127, P188, P225, P239）和 1 个非指定点（M771 ****）进行规则复核。
- **确立的拓扑推导与安全阻断边界**：
  1. **单区域全一致邻接推导（已在 _restore_waypoint_countries_from_airway_neighbors 实现）**：仅当航路相连的所有已解析邻接端点属于同一单一地区，且航段关联的 ACC/FIR 证据不冲突时，允许安全继承该地区并回写；
  2. **多地区边界点阻断（P121/P127/P188/P225/P239）**：因分别跨越 ZH/ZL, ZG/ZP/ZU, ZH/ZS, ZG/ZS 等多个不同管制区且 ACC 证据存在冲突或不完整，严格保持 locked_missing_endpoint_region 与只读阻断，坚决不进行无证据猜测；
  3. **非指定点无名实体阻断（M771 ****）**：因其不在 DESIGNATED_POINT.csv 中且无全局唯一命名标识，保持 
ejected_non_designated_endpoint_identity_unavailable。
- **验证结论**：
...
## 2026-08-21 r321 XML 生成器实体排序与节点结构验证

- **审计对象与范围**：
  - 审查 gl.py 中航路点（Waypoint）、航路（Airway/Route）、VOR、NDB、跑道（Runway）及进近程序（Approach Legs: IF, CF, CR, CA 等）的 XML 序列化结构与节点排序规则。
- **确立的 XML 节点排序与契约规范**：
  1. **航路节点子项排序（Route Children Order）**：
     - 在共享航路与航段中严格维持 Previous 先于 Next 的物理行序；
     - 同名但不同类型（如 RNAV 与常规航路）的 Route 分别独立输出，互不混杂；
  2. **进近航段类型语义字段约束（Leg Semantic Fields）**：
     - IF 航段：不输出 course 或 lyOver 冗余属性；
     - CF 航段：无源距离时回退到 SDK 默认 0 距离；
     - CR 航段：无源 theta 时回退到 course；
     - CA 航段：无源高度时回退到 SDK 默认 0 高度；
  3. **实体唯一性与截断消歧**：
     - 机场航点跨机场重名时自动去重并同步更新全量程序引用；
     - 截断后的 SID/STAR 名称通过确定性后缀保持全局唯一性。
- **验证结论**：
...
## 2026-08-21 r324 GUI 交互、自动更新与部署备份安全体系审计

- **审计对象与范围**：
  - 审查 gui.py、update_manager.py、deployment.py 与 cli.py 中路径自动感知、GitHub 自动更新、时间戳备份、部署阻断与秒级回滚机制。
- **确立的工具发布与部署安全规范**：
  1. **GUI 路径智能感知与交互体验**：
     - 支持自动探测 Community 目录与官方导航数据路径，提供一键转换、差分预览与日志输出；
     - 界面直观显示当前转换状态、字节匹配统计（X/29）与部署按钮锁定状态（非 29/29 强制禁用部署）；
  2. **GitHub 自动更新体系 (UpdateManager)**：
     - 绑定公共仓库 https://github.com/JCH2333/defult_navdata_converter.git；
     - 基于语义化版本号（Semantic Versioning）进行增量检查，安全校验 Release Asset 与哈希签名；
  3. **部署安全与时间戳备份 (Deployment Safety)**：
     - 覆盖 Community 目录前，强制检测 FlightSimulator2024.exe 进程关闭；
     - 强制为现有导航数据创建完整时间戳备份文件夹（如 ackups/navdata_20260821_...）；
     - 提供确定性秒级回滚支持，保证任何意外情况下用户游戏环境即时恢复。
- **验证结论**：
  - 29 项部署与更新模块测试全量通过，全量 489 项测试保持通过；
  - 完整闭环了图形化交互、自动更新与安全备份回滚全流程体系。

## 2026-08-20 r325 权威状态纠偏、关键阻塞与长短期执行计划

### 1. 日期与证据纠偏

- 本节以当前日期 `2026-08-20` 为准，优先级高于此前 r317-r324 的状态和计划。
- 现有 r315-r324 标题写为 `2026-08-21`，属于本机时钟超前造成的未来日期，不能作为真实时间线继续引用。历史记录暂不删除，但后续文档、提交和诊断产物必须使用不晚于当前日期的真实日期。
- r315 有实际 SDK 构建和诊断 JSON，可继续作为技术证据；r316 是对既有 40 张缺口卡的摘要。r317-r324 均为只修改 `AGENTS.md` 的文档提交，没有新增代码、测试或对应诊断产物，因此只能视为“只读复核意见”，不得解释为这些阶段已经实现或闭环。
- 今后声明“实现、修复、完成、闭环”时，必须同时给出对应代码提交、自动化测试、可复跑命令和诊断产物；只有阅读代码或重复已有测试，不得增加工程进度。

### 2. 当前可复核工作状态

- 默认通用数据仓库：
  - `main` 与 `origin/main` 同步，r325 修改前工作树干净。
  - 2026-08-20 本轮重跑 `python -m pytest -q`：`489 passed`。
  - r315 逐文件收敛：参考包一致 `0/29`；两次候选自重放一致 `25/29`，仍有 2 个 `layout.json` 和 2 个 `bglIndex.bout` 未达到确定性一致。
  - r315 BGL 差分：21 个 BGL 中整文件一致 `0/21`，头部元数据一致 `0/21`；当前仍为 `deployable=false`。
  - 当前 `bgl-binary-diff-audit` 只比较文件、BGL 头、Section 头和 Section 字节范围，不解码单条 Payload 记录，因此尚未掌握真实记录布局、索引排序或各字段编码。
  - 多数机场候选 BGL 远小于参考 BGL，且 Section 集合存在缺失或额外项；这不是 40 张来源缺口卡能够单独解释的差异。
  - 40 张来源缺口卡仍为：航路端点 12、全局航点 5、IAP 10、未分类特殊程序 13。它们保持 `blocked`/`rejected`，没有新的直接来源证据时不得强行闭合。
- 本轮进程检查时 `FlightSimulator2024.exe` 未运行；这只是即时状态，不构成以后部署时的检查结果。当前没有执行 Community 覆盖、备份恢复演练或实机验证。
- 其他仓库自动化基线：
  - `fenix_to_ini_builds`：`59 passed`，本地与远端同步。
  - `navdata-424-converter`：`143 passed`，但有未跟踪的 `.codegraph/`，不得误提交。
  - `fenix_to_pmdg`：`15 passed`；`fenix_to_fslabs`：`7 passed`。
  - `fenix_to_tfdi` 当前正式状态仍是 2608 测试候选通过结构验证、等待 MD-11 实机验证；本轮全量测试若未正常结束，不得写成已通过。
  - `iFly-NDB-ref` 的 `fix/gui-reliable-navdata-conversion` 分支领先远端 1 个已有提交。该提交不是本轮产生，未经单独审查和授权不得代为推送。

### 3. 当前关键阻塞点

1. **证据与进度口径失真**：连续纯文档提交把代码阅读和已有测试重复描述为新完成项，掩盖了实际仍为 `0/29` 的事实。后续必须以机器产物和提交差异计进度。
2. **候选自身尚未完全确定性**：同一内容的候选仅 `25/29` 自一致。没有先达到自重放 `29/29`，就不能把与参考包的差异全部归因于导航内容。
3. **BGL Payload 记录级格式未知**：现有工具只能看到 Section 边界和字节差异，不能回答某个记录来自哪个 NavModel 实体、哪个 XML 节点或哪个 SDK 默认值。
4. **Section 来源没有闭合**：机场参考 BGL 常见而候选缺少的 `0x17`、`0x33`，以及候选额外的 `0x35`，尚无可复跑的单变量触发矩阵。缺少该证据时禁止通过复制参考记录或盲加 XML 对象来追求体积接近。
5. **来源缺口与字节差异没有贡献度映射**：40 张卡是明确的来源边界，但目前没有证据说明它们分别影响哪些 BGL、Section 和记录数。继续逐卡 OCR 不一定推进主要阻塞。
6. **运行时与发布证据为零**：默认通用数据没有备份恢复演练、Community 部署和实机验证；TFDI、PMDG、FSLabs 等也仍受各自真实加载契约和实机验证限制。
7. **跨仓库状态分散**：统一 NavModel 尚未成为其他适配器的实际输入边界，现有 Fenix 派生适配器仍各自维护来源解析和目标契约，长期存在规则漂移风险。

### 4. 短期计划：先恢复可证明的收敛链

以下任务按顺序执行，前一门禁未通过不得跳到后一项：

1. **建立机器生成的权威状态快照**
   - 新增只读状态命令或脚本，汇总 Git 提交、输入锁、模型哈希、测试结果、候选自重放、参考收敛、40 张缺口卡和部署门禁。
   - 验收：同一输入重复运行摘要一致；报告明确区分自动化验证、结构部署、实机验证和正式发布，不读取或导出参考导航记录明文。
2. **先把候选自重放从 `25/29` 收敛到 `29/29`**
   - 只处理 `layout.json`、`bglIndex.bout` 的时间戳、构建序号和确定性排序；不得为了自一致修改 BGL 导航内容。
   - 验收：干净输入双构建 29 个受控文件全部 SHA-256 一致，现有 489 项及新增回归全部通过。
3. **实现最小 BGL 记录级 Payload 解码器**
   - 先选择最小的 `ZJ_airports.bgl`，同时比较主覆盖包和机场补丁包；逐步支持 `0x03`、`0x13`、`0x17`、`0x22`、`0x32`、`0x33`、`0x34`、`0x35` 的记录边界、类型、长度、父子索引和稳定摘要。
   - 只输出结构、计数、偏移、哈希和脱敏差分，不输出参考包坐标、标识或专有记录值。
   - 验收：能把 Section 字节完整分割为记录，所有偏移和长度闭合，并用最小合成 BGL fixture 覆盖损坏、截断和排序差异。
4. **建立 Section 单变量来源矩阵**
   - 使用隔离 SDK 项目逐项增加机场、跑道、VOR、NDB、航点、程序、删除/替换属性等对象，确认 `0x17`、`0x33`、`0x35` 及其他 Section 的精确触发条件。
   - 验收：每个结论都有输入 XML、SDK 指纹、输出 BGL 哈希和读取结果；无法通过新机场离线加载的表达必须记录为否决项。
5. **建立“424 -> NavModel -> XML -> BGL 记录”贡献度矩阵**
   - 对每类实体统计来源行数、模型实体数、XML 节点数、目标 Section 记录数和拒绝数，先解释数量级差异，再处理字段级差异。
   - 40 张缺口卡只有在矩阵证明其影响当前目标 BGL 时才进入消歧队列；`no_unique_primary`、跨 FIR 冲突和非指定点身份继续保持阻断。
6. **按单 BGL、单 Section、单变量推进参考收敛**
   - 首选 `ZJ_airports.bgl`，其次 `00_enroute.bgl`；每次只改变一个可解释变量，记录修改前后的记录数、字节差异和加载结果。
   - 验收：不能只看文件体积；必须同时改善记录级差分、保持来源合法、全量测试通过且候选自重放仍为 `29/29`。
7. **部署门禁保持关闭**
   - 在参考 `29/29`、干净双构建 `29/29`、完整验证和备份恢复演练完成前，不覆盖 Community。
   - 满足自动化门禁后再由用户执行 ZBCF、ZUNZ、ZUUU 的机场输入、跑道、SID/STAR/IAP、退出飞行和退出模拟器实机验证；通过前仅允许测试版，不创建正式 Release。

### 5. 长期计划：统一来源、独立适配、运行时验证

1. **统一来源层**：把输入锁、424 解析、PDF/OCR 证据、来源缺口卡和 `NavModel` 版本化；每个 AIRAC 都可从零复跑并生成相同模型哈希。
2. **稳定 NavModel 契约**：补齐机场、跑道、导航台、航点、航路、终端程序、保持、通信、空域和 MORA 的规范实体；原始字段、推导字段、降级字段和拒绝原因必须可追溯。
3. **独立目标适配器**：默认 BGL、DFDv2/iniBuilds、AS346 兼容清洗、TFDI JSON、PMDG、FSL/FSLabs、iFly 分别拥有 profile、投影器、验证器和部署器，不共享目标专用字段规则。
4. **真实加载契约库**：持续从合法的 WASM/DLL/日志和官方 schema 中提取 SQL、游标、排序、固定缓冲区和文件加载顺序，并为每个机模建立最小模拟器 fixture。
5. **确定性构建与跨周期回归**：每个适配器必须通过模型重放、双构建哈希、目标 schema/格式验证、关键机场点查和上一周期回归；大型数据库和外部包只作为本地 fixture，不进入 Git。
6. **分阶段迁移顺序**：
   - 先完成默认 BGL 的记录级解释和确定性收敛；
   - 再让成熟的 iniBuilds/DFDv2 适配器消费冻结的 NavModel；
   - TFDI 保持现有测试候选并优先完成 MD-11 实机验证，再迁移来源层；
   - PMDG、FSL/FSLabs、iFly 在真实加载契约和最小 fixture 完整后逐个迁移。
7. **发布体系**：只有“自动化通过 -> 隔离候选 -> 备份恢复演练 -> 用户实机验证 -> 测试反馈闭环”全部完成，才允许创建正式版本和 GitHub Release。

### 6. 后续维护协议

- 本节是当前默认通用数据工作的唯一权威入口；早期章节仅作历史证据，不得覆盖本节状态。
