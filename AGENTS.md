...
- 最近全量测试：`519 passed`；最新阶段 r394 IAP 未决项来源直接证据审计。
...
- r390-r394：新增 package-derived-metadata-audit、airway-endpoint-audit、default-gap-cards-audit、unclassified-procedure-cards-audit 与 iap-primary-source-audit。元数据差异依赖编译过程，未授权手工修改；航路 10 个未决端点缺少唯一 424 区域；13 张未分类程序缺口卡与 10 张 IAP 未决缺口卡（ZBAD:R29R、ZJSY:I08-X、ZSNJ:I25、ZSOF:R15/R33、ZSWY:I03、ZUAL:I15、ZYDD:R01/R01-Y、ZYTL:R10）图页直接证据核验确认均缺少唯一直接主进近/类别锚点证据，全部保持严格拒绝。模型/适配器不变。证据：diagnostics/r390-* 至 r394-*。
