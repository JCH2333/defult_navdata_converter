...
| 机场来源对象盘点与 SDK 表达 | 未开始 | 每类 424 对象均有来源、归属、可表达性、拒绝原因和单变量探针结论 |
...
3. **先建立仅来源侧的机场对象库存。** 只读取 `NavModel`、2608 424 CSV/PDF、SDK XSD 和候选 XML，不读取参考 BGL 导航记录。对机场、跑道、终端航点、ILS、程序、等待航线、通信、机场关联 NDB/VOR 等逐类记录数量、机场归属、来源文件、SDK 可表达类型、目标作用域和拒绝原因；库存本身必须可序列化、可比较并带最小 fixture。
4. **以库存驱动单变量 SDK 探针。** 每次选择一个能由 424 独立证明、尚未验证的机场对象，在 `airport_subset_probe.py` 中构建最小输入。报告必须保存 XML、Package Tool 轨迹、BGL 头部/节表、读取器登记、输出哈希和结论。不得再尝试仅靠 `Ndb`、`onlyAddIfReplace`、根节点终端点重复或参考节表计数解释机场 BGL。
