...
- 提供 `airway-connection-shape-probe`：用最小合成 Route 连接形态确认 SDK 只有同时收到 `Next` 和 `Previous` 才会生成航路行，并回读端点、包围盒、片段和序号；不读取参考成品或修改任何转换候选。
...
python -m fenix_default_navdata.cli reference-template-source-audit `
  --reference "F:\我的世界动画\AI项目\导航数据\424源数据\2608\Default navdata 2608R1" `
...
python -m fenix_default_navdata.cli airway-connection-shape-probe `
  --output diagnostics\airway-connection-shape-probe `
  --bglcomp "C:\MSFS 2024 SDK\Tools\bin\fspackagetool.exe"
...
python -m fenix_default_navdata.cli airway-endpoint-card-audit `
  --raw "F:\我的世界动画\AI项目\导航数据\424源数据\2608\2608" `
  --model output\intermediate-2608-r187-navaid-label-replay.json.gz `
  --ident P225 `
...
python -m fenix_default_navdata.cli unclassified-procedure-card-audit `
  --model output\intermediate-2608-r187-navaid-label-replay.json.gz `
  --card ZGBS:RNP-0:12:0 `
  --output diagnostics\unclassified-procedure-card-audit.json
...
- `sdk-bgl-expression-matrix-audit` 汇总来源清单、航路投影矩阵、SDK 连接/子节点顺序探针和 BGL 节审计；可用 `--offset-threshold-matrix` 接入历史 `offset-threshold-matrix-v1` 摘要。探针完成且未修改模型/候选时，会将该候选归档为 `rejected_after_probe`，避免把已验证变量重复列入后续计划。
...
- `airport-document-source-audit` 消费固定 OCR 运行时生成的机场 PDF JSON，按目标 ICAO 输出源文件哈希、页面覆盖和命中摘要哈希，不保存 OCR 正文；命中也不会自动授权投影，必须另有结构化字段和目标加载契约证据。该步骤可复用于其他目标格式的来源缺口审计。
...
- `unclassified-procedure-card-audit` 以缺口卡精确键只读审计一段未分类程序。它验证来源 PDF SHA-256、统计标签在同页直接文本的出现次数，并列出程序类别标题的直接文本事实。只有标签恰好一次且与唯一类别标题位于同一文本行时才报告来源类别；相邻标题、航段顺序、OCR 或标签形态都不能推断类别。
