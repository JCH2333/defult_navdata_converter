...
def test_bgl_iap_chart_roles_reuse_rnp_subset_direct_role_consensus(
    ...
    primary = ProcedureSegment(
        "ZBCF", "R03", "approach", "03", "", (
            ChartTerminalLeg(
                "R03", "03", "IF", "IF01", "IF IF01 RNP0.3", sequence=1,
                fix_region="ZB", fix_type="TERMINAL_WAYPOINT",
                fix_latitude=35.10, fix_longitude=105.0,
            ),
            ChartTerminalLeg(
                "R03", "03", "TF", "RW03", "TF RW03 RNP0.3", sequence=2,
                fix_region="ZB", fix_type="TERMINAL_WAYPOINT",
                fix_latitude=35.05, fix_longitude=105.0,
            ),
        ), source,
    ...
    model.procedure_charts.extend([
        ...
        ProcedureChart(
            "ZBCF", "ZBCF-5L.pdf", 1, "instrument-approach-index",
            "LOC/DME RWY03", "text", (), ("03",), (), (), (), source,
        ),
    ])
    ...
    output = tmp_path / "iap-rnp-subset-consensus.xml"
    write_bglcomp_xml(model, DEFAULT_CYCLE, output, scope="airports")
