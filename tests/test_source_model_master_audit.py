...
from fenix_default_navdata.source_model_master_audit import (
...
def test_source_model_master_audit(tmp_path: Path) -> None:
    raw = tmp_path / "raw"
    raw.mkdir()
    ...
    report = audit_source_model_master(raw, model)

    assert report["diagnostic"] == "source-model-master-audit-v1"
    assert report["read_only"] is True
    assert report["summary"]["master_pipeline_verified"] is True
    assert report["summary"]["root_csv_total"] == 33
    assert report["summary"]["unclassified_csv_total"] == 0

    out_file = tmp_path / "out.json"
    write_source_model_master_audit(out_file, report)
